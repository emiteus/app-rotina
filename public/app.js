...
function renderExtrato(data) {
  ...
  lista.innerHTML = dias.map((dia) => {
    ...
    const rows = itens.map((t) => {
      ...
      const terceiro = t.pago_terceiro && t.terceiro_nome
        ? `<span class="extrato-terceiro" title="Pagamento por terceiro">👤 ${escapeHtml(t.terceiro_nome)}</span><span class="sep">·</span>`
        : '';
      return `
