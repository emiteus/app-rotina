...
async function rankingDoDia(userIdAtual) {
  ...
  const rows = await all(`
    ...
    ORDER BY concluidas DESC, total DESC, u.nome ASC
  `, [hoje]);
  ...
  if (jogadores.length >= 2) {
    ...
    if (top.concluidas > second.concluidas) {
      lider = { id: top.id, nome: top.nome, diff: top.concluidas - second.concluidas };
    } else if (top.concluidas === second.concluidas && top.concluidas > 0) {
      lider = { id: null, nome: null, diff: 0, empate: true };
    }
  } else if (jogadores.length === 1 && jogadores[0].concluidas > 0) {
    lider = { id: jogadores[0].id, nome: jogadores[0].nome, diff: jogadores[0].concluidas };
  }
