...
async function rankingDoDia(userIdAtual) {
  ...
  const jogadores = rows.map((r) => {
    ...
    const pct = total > 0 ? Math.min(100, Math.round((concluidas / total) * 100)) : 0;
    return {
      id: r.id,
      nome: r.nome,
      cor: r.cor || '#f97316',
...
async function rankingHistorico(userIdAtual, opts = {}) {
  ...
  for (const r of rows) {
    ...
    const total = Number(r.total) || 0;
    const concluidas = Number(r.concluidas) || 0;
    const prev = byDay.get(dia).get(r.user_id);
    if (prev && prev.concluidas >= concluidas && prev.total >= total) continue;
