...
async function checkinHabito(titulo = 'Academia', userId) {
  ...
  let task = await get(
    `SELECT * FROM tasks
     WHERE user_id = $1 AND data_reset::date = $2::date
       AND (titulo = $3 OR titulo ILIKE $4)
     ORDER BY CASE WHEN titulo = $3 THEN 0 ELSE 1 END, concluida ASC, data_criacao DESC
     LIMIT 1`,
    [userId, hoje, nome, `%${nome}%`]
  );
  ...
  const id = uuid();
  await run(
    `INSERT INTO tasks (id, titulo, descricao, prioridade, categoria, data_reset, concluida, concluida_em, user_id)
     VALUES ($1, $2, '', $3, $4, $5, true, CURRENT_TIMESTAMP, $6)`,
    [id, nome, habito.prioridade || 'media', habito.categoria || 'saude', dataResetSql(hoje), userId]
  );
  task = await get(`SELECT * FROM tasks WHERE id = $1`, [id]);
  return { ok: true, ja: false, criada: true, task, titulo: nome };
}

async function resumoHabito(titulo = 'Academia', userId) {
  const habito = resolverHabito(titulo);
  const nome = habito.titulo;
  const hoje = hojeStr();
  const mes = await get(
    `SELECT COUNT(*) FILTER (WHERE concluida)::int AS concluidas, COUNT(*)::int AS total
     FROM tasks
  ...
  const hojeTask = await get(
    `SELECT id, concluida, concluida_em FROM tasks
     WHERE user_id = $1 AND data_reset::date = $2::date AND (titulo = $3 OR titulo ILIKE $4)
     ORDER BY CASE WHEN titulo = $3 THEN 0 ELSE 1 END, data_criacao DESC LIMIT 1`,
    [userId, hoje, nome, `%${nome}%`]
  );
  return {
    titulo: nome,
...
async function listarHabitos(userId) {
  const lista = [];
  for (const h of HABITOS_PADRAO) {
    lista.push(await resumoHabito(h.titulo, userId));
  }
  return { habitos: lista };
}

async function limparHabitosRemovidos(userId) {
  let desativadas = 0;
  let apagadas = 0;
  for (const titulo of HABITOS_REMOVIDOS) {
    const r1 = await run(
      `UPDATE tarefas_recorrentes SET ativa = false WHERE user_id = $1 AND titulo ILIKE $2 AND ativa = true`,
      [userId, titulo]
    );
    desativadas += r1?.rowCount || 0;
    const r2 = await run(
      `DELETE FROM tasks WHERE user_id = $1 AND titulo ILIKE $2
...
async function garantirRecorrentesHabitos(userId) {
  if (!userId) return { criadas: 0, desativadas: 0, apagadas: 0 };
  const limpeza = await limparHabitosRemovidos(userId).catch(() => ({ desativadas: 0, apagadas: 0 }));
  let criadas = 0;
  for (const h of HABITOS_PADRAO) {
    const existe = await get(
      `SELECT id FROM tarefas_recorrentes WHERE user_id = $1 AND titulo ILIKE $2 AND ativa = true LIMIT 1`,
      [userId, h.titulo]
    );
    if (existe) continue;
    const id = uuid();
    await run(
      `INSERT INTO tarefas_recorrentes (id, titulo, descricao, prioridade, categoria, frequencia, dias_semana, user_id)
       VALUES ($1, $2, '', $3, $4, 'diario', $5, $6)`,
      [id, h.titulo, h.prioridade, h.categoria, h.dias_semana, userId]
    );
    criadas++;
  }
...
async function garantirRecorrenteAcademia(userId) {
  return garantirRecorrentesHabitos(userId);
}

async function garantirRecorrentesHabitosTodos() {
  const users = await all(`SELECT id FROM usuarios WHERE ativo = true`);
  let criadas = 0;
  for (const u of users) {
    const r = await garantirRecorrentesHabitos(u.id);
    criadas += r.criadas || 0;
  }
  return { usuarios: users.length, criadas };
}

/** Consistência de horário de conclusão (últimos N dias). */
async function analisarConsistencia(dias = 30, userId) {
  const n = Math.min(Math.max(Number(dias) || 30, 7), 90);
  const porTitulo = await all(
    `SELECT titulo, COUNT(*)::int AS vezes,
     ...
     FROM tasks
     WHERE user_id = $1 AND concluida = true AND concluida_em IS NOT NULL
       AND concluida_em >= CURRENT_DATE - ($2::int * INTERVAL '1 day')
     GROUP BY titulo HAVING COUNT(*) >= 2 ORDER BY COUNT(*) DESC LIMIT 20`,
    [userId, n]
  );

  const porHora = await all(
    `SELECT EXTRACT(HOUR FROM concluida_em)::int AS hora, COUNT(*)::int AS n
     FROM tasks WHERE user_id = $1 AND concluida = true AND concluida_em IS NOT NULL
       AND concluida_em >= CURRENT_DATE - ($2::int * INTERVAL '1 day')
     GROUP BY EXTRACT(HOUR FROM concluida_em) ORDER BY hora`,
    [userId, n]
  );
...
module.exports = {
  ...
  garantirRecorrentesHabitosTodos,
