...
async function getAttracioneSnapshot() {
  ...
  try {
    ...
    // Vídeos publicados HOJE por participante (Mateus/Erik etc.)
    let hoje = null;
    if (planilha?.hoje) {
      hoje = {
        data: planilha.hoje.data,
        total_videos: planilha.hoje.total_videos,
        total_views: planilha.hoje.total_views,
    ...
    return {
      ...
      projeto: 'Attracione',
      nota:
        'Competição de cortes Mateus/Erik. "quantos reels postamos hj" → use hoje.por_pessoa (NÃO SocialHub). Ranking da comp → ranking.top (views+vídeos).',
      operacao: {
