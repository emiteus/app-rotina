 ...
 */
/**
 * Toda resposta sai daqui pro WhatsApp, pro PC e pra web: passa pelo filtro de conversa de gente
 * (sem nome de comando, campo técnico ou JSON — pedido do Mateus em 24/09/2026).
 */
async function runJarvisTurn(opts = {}) {
  const out = await runJarvisTurnInner(opts);
  if (out && typeof out.resposta === 'string') out.resposta = require('./nl/humanize').humanizeReply(out.resposta);
  return out;
}

async function runJarvisTurnInner(opts = {}) {
