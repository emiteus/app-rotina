...
const ensureApprovalsTable = once(async () => {
  await run(`
...
function summarizeAcoes(acoes) {
  ...
  return (acoes || [])
    .map((a) => {
      // Em português: "apagar a transação (Mercado)", nunca "deletar_transacao"
      const t = a && a.tipo ? label(a.tipo) : 'fazer isso';
      const hint =
        a.titulo ||
        a.nome ||
        a.search ||
...
/** Assinatura de uma ação (tipo + args, sem ordem de chave) — pra achar pedido repetido. */
function acaoSignature(a) {
  const norm = (v) => {
...
/** Ações aprovadas por você nos últimos `ms` (padrão 5 min). */
async function recentlyApprovedSignatures(userId, ms = 5 * 60 * 1000) {
  await ensureApprovalsTable();
  const { all } = require('../host').requireLib('db');
  const rows = await all(
    `SELECT acoes, resolvido_em FROM jarvis_approvals
     WHERE user_id = $1 AND status = 'approved' AND resolvido_em > NOW() - ($2::int * INTERVAL '1 millisecond')`,
    [userId, Math.round(ms)]
  );
  const out = new Map();
  for (const r of rows || []) {
    const acoes = Array.isArray(r.acoes) ? r.acoes : JSON.parse(r.acoes || '[]');
    for (const a of acoes) out.set(acaoSignature(a), r.resolvido_em);
  }
  return out;
}
