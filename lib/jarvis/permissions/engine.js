...
async function holdForApproval(userId, gatedAcoes, { channel } = {}) {
  // Reusa o id pendente do canal, mas troca as ações pelo pedido atual —
  // senão o SIM executaria o pedido anterior.
  const existing = await getPendingApproval(userId, channel);
  if (existing) {
    const updated = await replaceApprovalAcoes(existing.id, userId, gatedAcoes, {
