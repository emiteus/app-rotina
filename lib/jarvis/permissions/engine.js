 ...
 * WhatsApp: HITL ligado por padrão (high+critical pedem SIM). Opt-out JARVIS_HITL_WHATSAPP=0.
...
/**
 * HITL por canal. WA também pede SIM: o Jarvis lê conteúdo de terceiros
 * (extrato, páginas, DMs), então high não roda direto em nenhum canal.
 */
function hitlAppliesToChannel(channel) {
  if (!hitlEnabled()) return false;
  const ch = normalizeChannel(channel);
  if (ch === 'whatsapp') {
    return process.env.JARVIS_HITL_WHATSAPP !== '0';
  }
  return true;
}
...
const BARE_YES = /^(sim|confirma|confirmar|confirmo|yes|aprovado|aprova|aprovo|pode(?: sim| fazer| mandar| ir| seguir| rodar)?|sim,? pode|pode sim,? jarvis|manda(?: ver| bala)?|bora|vai|faz(?: isso| a[ií])?|isso|claro|positivo|ok,? pode|sim senhor|fechou|com certeza)$/i;
const BARE_NO = /^(n[aã]o|cancela|cancelar|nega|nope|no|n[aã]o precisa|n[aã]o,? obrigado|deixa|deixa (?:pra|para) l[aá]|esquece|negativo|para|n[aã]o faz)$/i;
