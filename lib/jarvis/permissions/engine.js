...
function parseApprovalReply(mensagem) {
  ...
  // "pode" solto fica de fora: é comum demais em conversa ("pode sim", "pode ser").
  m = t.match(/^(sim|confirma|confirmar|yes|aprovado|aprova)\s*[!.]*$/i);
...
function formatApprovalAsk(approval) {
  ...
  // Conversa de gente: pergunta direta; o código fica no fim, só pra quando houver mais de um pedido
  return `⚠️ Posso ${summary}?\nResponde **SIM** pra confirmar ou **NÃO** pra cancelar. _(pedido ${id})_`;
}
