 ...
 *              { type:'reply', id, text, approval?, images?, audio?, audioPending? }
 *              { type:'reply_audio', id, audio|null } — voz do Jarvis chegando depois do texto
 *              { type:'tool', id, tool, args } (só tools pc_*, tv_* e soc_* registradas)
 *              { type:'error', id?, message } · { type:'pong' } · { type:'notify', text }
 *
 * Celular (Fase 5.5, página instalada): o navegador não manda header no WebSocket, então o token vai
 * como subprotocolo ("jarvis-device", "<token>") — nunca na URL (URL vai pra log). O celular conversa
 * (texto/voz) e manda { type:'push_sub', sub } pra receber aviso; nunca recebe tool nem aviso falado.
 */
...
// Voz do Jarvis (a mesma do WhatsApp: Gemini "Charon"; OpenAI na frente se tiver crédito).
// O texto sai na hora e a voz vem depois (reply_audio); passou do prazo, o aparelho fala com a voz local.
const VOICE_TTS_BUDGET_MS = Number(process.env.JARVIS_DESKTOP_TTS_MS || 25000);
...
/** id -> { resolve, timer, deviceId } — avisos esperando o PC dizer se falou */
const notifyWaits = new Map();
let notifySeq = 0;
let toolSeq = 0;
const TOOL_TIMEOUT_MS = 15000;

const SUBPROTOCOL = 'jarvis-device';

function bearer(req) {
  const h = String((req.headers && req.headers.authorization) || '');
  const m = h.match(/^Bearer\s+(\S+)$/i);
  if (m) return m[1];
  // Navegador: new WebSocket(url, ['jarvis-device', token]) — o servidor responde 'jarvis-device'
  const protos = String((req.headers && req.headers['sec-websocket-protocol']) || '')
    .split(',')
    .map((s) => s.trim());
  if (protos[0] === SUBPROTOCOL && /^jdv_[A-Za-z0-9_-]{20,}$/.test(protos[1] || '')) return protos[1];
  return null;
}

/** Aparelho que executa comando e fala aviso (PC). Celular só conversa. */
const isPc = (ws) => !ws.jarvisDevice || ws.jarvisDevice.kind !== 'phone';
...
function replyPayload(id, out) {
  ...
  return {
    ...
    text: String((out && out.resposta) || '')
      // A etiqueta de abertura traz a origem junto: "[CONTEÚDO EXTERNO — …] (pc_screen)"
      .replace(/\[\/?CONTEÚDO EXTERNO[^\]]*\](?:[ \t]*\([\w.-]{1,60}\))?\n?/g, '')
      .trim(),
    approval: pending
...
function createGateway(deps = {}) {
  ...
  function onConnection(ws, device) {
    ...
    send(ws, {
      ...
      ...(isPhone ? { vapidKey: require('./phone-push').vapidPublicKey() } : {})
    });
    ...
    ws.on('message', (raw) => {
      ...
      if (msg.type === 'notify_ack') {
        // PC diz se falou o aviso (você no PC) ou não (longe/silêncio) → quem chamou decide o WhatsApp
        const w = notifyWaits.get(String(msg.id || ''));
        if (!w || w.deviceId !== device.id) return;
        notifyWaits.delete(w.id);
        clearTimeout(w.timer);
        w.resolve({ delivered: true, spoken: msg.spoken === true, deviceId: device.id });
        // Vai falar: manda a voz do Jarvis (a do WhatsApp); sem ela a tempo, o PC usa a voz local
        if (msg.spoken === true && w.speech && msg.localVoice !== true) {
          tts(w.speech)
            .catch(() => null)
            .then((sp) =>
              send(ws, { type: 'reply_audio', id: w.id, audio: sp && sp.base64 ? { mime: sp.mime, base64: sp.base64 } : null })
            );
        }
        return;
      }
      if (msg.type === 'push_sub') {
        // Celular liga/desliga os avisos (inscrição conferida: só serviço de push oficial)
        if (!isPhone) return;
        const pp = require('./phone-push');
        const sub = msg.sub == null ? null : pp.cleanSubscription(msg.sub);
        if (msg.sub != null && !sub) return send(ws, { type: 'push_state', on: false, erro: 'inscrição inválida' });
        (deps.setPush || store.setDevicePush)(device.id, sub)
          .then(() => send(ws, { type: 'push_state', on: !!sub }))
          .catch(() => send(ws, { type: 'push_state', on: false, erro: 'não consegui salvar' }));
        return;
      }
      ws.lastActive = Date.now();
      if (msg.type !== 'chat' && msg.type !== 'voice' && msg.type !== 'briefing') {
      ...
      const isBriefing = msg.type === 'briefing';
      const text = isVoice ? '' : isBriefing ? '[briefing]' : String(msg.text || '').trim();
      let audio = null;
      if (isVoice) {
      ...
      chain = chain
        .then(async () => {
          let message = text;
          // Briefing do dia (Fase 5.2): o PC pede, o servidor monta o pedido com os avisos do sistema
          if (isBriefing) message = await (deps.briefingPrompt || briefingPrompt)(device.userId);
          ...
          if (isBriefing) payload.briefing = true;
          // Voz entra, voz sai: o texto vai já, a voz do Jarvis logo depois (o aparelho espera por ela)
          // localVoice: o PC fala com a voz dele (Piper, 25/09/2026) — não gera a Charon (2,5–22 s)
          const wantsVoice = !!((audio || isBriefing) && payload.text) && msg.localVoice !== true;
          if (wantsVoice) payload.audioPending = true;
          send(ws, payload);
          timing.textMs = Date.now() - t0;
          const logTiming = (spoke) => {
            if (!audio) return;
            console.log(
              JSON.stringify({
                tag: 'jarvis.voice',
          ...
          if (!wantsVoice) return logTiming('pc');
          // A voz é gerada fora da fila: a próxima mensagem não espera o áudio desta
          const tTts = Date.now();
          tts(payload.text)
            .catch(() => null)
            .then((speech) => {
              timing.ttsMs = Date.now() - tTts;
              const ok = !!(speech && speech.base64);
              send(ws, { type: 'reply_audio', id, audio: ok ? { mime: speech.mime, base64: speech.base64 } : null });
              logTiming(ok ? 'server' : 'pc');
            });
        })
        .catch((err) => {
...
function requestTool(userId, tool, args = {}, { timeoutMs = TOOL_TIMEOUT_MS } = {}) {
  const mine = [...connections.values()].filter(
    (ws) => ws.jarvisDevice && ws.jarvisDevice.userId === String(userId) && ws.readyState === 1 && isPc(ws)
  );
  if (!mine.length) {
...
/**
 * Pedido do briefing do dia (Fase 5.2). Vai pelo turno normal (agenda, tarefas e finanças já
 * entram no contexto) + os avisos de sistema de agora, já em frase de gente.
 */
async function briefingPrompt(userId) {
  let avisos = [];
  try {
    const notes = await require('../events/bus').runProactiveSweep(userId);
    avisos = notes.filter((n) => n && n.level === 'warn').map((n) => n.speech || n.text);
  } catch (_) {
    /* sem avisos */
  }
  ...
  return (
    `[Briefing pedido pelo Jarvis Desktop ao ligar o PC] Começa com "${saudacao}" e me dá o briefing de hoje, ` +
    'FALADO, em até 70 palavras e frases curtas, sem listas: compromissos e alarmes de hoje, tarefas importantes pendentes, ' +
    'contas ou DAS vencendo nos próximos 3 dias. ' +
    (avisos.length ? `Inclui estes avisos do sistema: ${avisos.join(' ')} ` : 'Os sistemas estão ok. ') +
    'Não faça nenhuma ação, só conte.'
  );
}

/**
 * Aviso falado no PC (Fase 5.1). Manda pro PC mais recente do usuário e espera ele dizer se falou.
 * spoken=false (você longe do PC, horário de silêncio, avisos desligados) → quem chamou usa o WhatsApp.
 * @returns {Promise<{ delivered: boolean, spoken: boolean, deviceId?: string }>}
 */
function speakToUser(userId, { text, speech = null, kind = 'aviso' } = {}, { waitMs = 6000 } = {}) {
  const mine = [...connections.values()].filter(
    (ws) => ws.jarvisDevice && ws.jarvisDevice.userId === String(userId) && ws.readyState === 1 && isPc(ws)
  );
  if (!mine.length) return Promise.resolve({ delivered: false, spoken: false });
  mine.sort((a, b) => (b.lastActive || 0) - (a.lastActive || 0));
  const ws = mine[0];
  const id = `n${Date.now().toString(36)}${(notifySeq++).toString(36)}`;
  return new Promise((resolve) => {
    const timer = setTimeout(() => {
      notifyWaits.delete(id);
      resolve({ delivered: false, spoken: false });
    }, waitMs);
    const fala = String(speech || text || '').slice(0, 600);
    notifyWaits.set(id, { id, deviceId: ws.jarvisDevice.id, timer, resolve, speech: fala });
    // voiceFollows: se o PC for falar, a voz do Jarvis vem depois em reply_audio (mesmo id)
    send(ws, { type: 'notify', id, text: String(text || '').slice(0, 1500), speech: fala, kind, wantAck: true, voiceFollows: true });
  });
}
