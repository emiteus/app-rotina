...
function speakToUser(userId, { text, speech = null, kind = 'aviso' } = {}, { waitMs = 6000 } = {}) {
  ...
  return new Promise((resolve) => {
    ...
    notifyWaits.set(id, { id, deviceId: ws.jarvisDevice.id, timer, resolve });
    send(ws, { type: 'notify', id, text: String(text || '').slice(0, 1500), speech: String(speech || text || '').slice(0, 600), kind, wantAck: true });
  });
}
