...
// Tipo do aparelho (Fase 5.5): celular conversa e recebe aviso, mas nunca executa comando de PC
const KINDS = new Set(['pc', 'phone']);
...
const ensureTables = once(async () => {
  ...
  `);
  await run(`ALTER TABLE jarvis_devices ADD COLUMN IF NOT EXISTS kind TEXT NOT NULL DEFAULT 'pc'`);
  // Inscrição Web Push do celular (endpoint + chaves; não dá acesso a nada além de mandar aviso)
  await run(`ALTER TABLE jarvis_devices ADD COLUMN IF NOT EXISTS push_sub TEXT`);
});
...
async function redeemPairCode(code, name, kind = 'pc') {
  await ensureTables();
  ...
  const safeKind = KINDS.has(kind) ? kind : 'pc';
  const fallback = safeKind === 'phone' ? 'Celular' : 'PC';
  const safeName = String(name || fallback).replace(/[^\p{L}\p{N} ._-]/gu, '').trim().slice(0, 40) || fallback;
  await run(
    `INSERT INTO jarvis_devices (id, user_id, name, token_hash, kind) VALUES ($1,$2,$3,$4,$5)`,
    [deviceId, row.user_id, safeName, sha256(token), safeKind]
  );
  console.log(JSON.stringify({ tag: 'jarvis.device', event: 'paired', deviceId, userId: row.user_id, kind: safeKind }));
  return { deviceId, token, name: safeName, userId: row.user_id, kind: safeKind };
}
...
/** Inscrição Web Push do celular (null = desligou os avisos). */
async function setDevicePush(deviceId, sub) {
  await ensureTables();
  await run(`UPDATE jarvis_devices SET push_sub = $2 WHERE id = $1 AND revoked_at IS NULL`, [
    deviceId,
    sub ? JSON.stringify(sub) : null
  ]);
}

/** Celulares do usuário com aviso ligado. */
async function phonePushTargets(userId) {
  await ensureTables();
  const rows = await all(
    `SELECT id, push_sub FROM jarvis_devices
     WHERE user_id = $1 AND kind = 'phone' AND revoked_at IS NULL AND push_sub IS NOT NULL`,
    [userId]
  );
  return (rows || [])
    .map((r) => {
      try {
        return { deviceId: r.id, sub: JSON.parse(r.push_sub) };
      } catch (_) {
        return null;
      }
    })
