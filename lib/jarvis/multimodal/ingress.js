...
function wantsLayoutImageEdit(userText, caption) {
  ...
  // "reproduz esse layout" já implica gerar; "só que / trocando" reforça
  return true;
}
...
async function ingressToText({ text, mediaMeta, fetchBase64 = true }) {
  ...
  if (understood) {
    if (layoutMode) {
      const userAsk = mediaMeta.caption || baseText || '';
      const layoutObj = parseLayoutJson(understood);
      const formatted = formatLayoutBlueprint(layoutObj || understood, {
        fileName: mediaMeta.fileName
      });

      // MCU: gera imagem travada na referência (não inventa VS/cinematic)
      if (wantsLayoutImageEdit(baseText, mediaMeta.caption)) {
        try {
          const { generateImage, buildLockedReproducePrompt, creativeEnabled } = require('./creative');
          if (creativeEnabled()) {
            const canRefImage =
            ...
            const cleanB64 = String(mediaB64 || '')
              .replace(/^data:[^;]+;base64,/, '')
              .replace(/\s+/g, '');
            if (canRefImage && cleanB64) {
              const prompt = buildLockedReproducePrompt(layoutObj, userAsk);
