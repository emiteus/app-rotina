...
async function geminiUnderstand({
  ...
  // Áudio: pedido reserva. O mesmo áudio leva ~1,5 s e às vezes 9–22 s (medido 23/09/2026).
  // Escalonado a cada 2 s: flash-lite, flash-lite de novo (costuma cair em servidor rápido), 3.5-flash
  // sem "pensamento". O primeiro que voltar ganha. Mediana medida: 1,5 s (antes 3,6–5 s).
  if (kind === 'audio' && modelList.length > 1) {
    ...
    const once = (model) => async (signal) => {
      ...
      const resp = await axios.post(
        ...
        {
          contents: [{ role: 'user', parts: [{ text: prompt }, { inlineData: { mimeType, data: cleanB64 } }] }],
          generationConfig: {
            temperature: 0.15,
        ...
        { timeout: 15000, signal, headers: { 'x-goog-api-key': process.env.GEMINI_API_KEY } }
      );
      const text = (resp.data?.candidates?.[0]?.content?.parts || []).map((p) => p.text || '').join('').trim();
      if (!text) return null;
      console.log(
        JSON.stringify({ tag: 'jarvis.multimodal', event: 'understood', provider: 'gemini', model, kind, durationMs: Date.now() - t0, chars: text.length })
      );
      return text;
    };
    try {
      // Reserva em modelos diferentes (24/09/2026: 3.5-flash-lite levou 18 s pra ouvir e as 2 cópias dele
      // travaram juntas; flash-lite-latest ouvia em 1,1 s). A cada 2 s entra o próximo; o primeiro ganha.
      const race = [...new Set([modelList[0], 'gemini-flash-lite-latest', 'gemini-3.5-flash', 'gemini-flash-latest'])].filter(Boolean);
      const starters = race.map(once);
      // Reserva FORA do Google (Whisper da OpenAI) na 3ª vaga (~4 s): se o Google inteiro engasgar,
      // alguém de fora ouve. Sem crédito/chave: pula sem custo (openai-health marca fora por 10 min).
      if (require('./openai-health').available()) {
        starters.splice(2, 0, (signal) => whisperTranscribe(cleanB64, mimeType, { signal }));
        race.splice(2, 0, 'whisper');
      }
      const text = await hedge(starters, {
        delayMs: 2000,
        onHedge: (i) => console.log(JSON.stringify({ tag: 'jarvis.multimodal', event: 'stt_hedge', to: race[i] }))
      });
      if (text) return text;
    } catch (err) {
      console.log(JSON.stringify({ tag: 'jarvis.multimodal', event: 'gemini_fail', kind, error: err.message, status: err.response?.status || null }));
    }
    // Os 4 modelos já tentaram: repetir um por um (45 s cada) só deixava a voz presa por 1 min.
    // Devolve nada e quem chamou diz "não entendi, fala de novo" na hora.
    return null;
  }
 ...
 */
async function whisperTranscribe(base64, mime, { signal = null } = {}) {
  if (process.env.JARVIS_WHISPER === '0') return null;
  const openaiHealth = require('./openai-health');
  const key = openaiHealth.openaiKey();
  if (!key || !base64 || openaiHealth.isDown()) return null;
  ...
  try {
    ...
    if (!resp.ok) {
      openaiHealth.noteHttpFailure(resp.status, JSON.stringify(data));
