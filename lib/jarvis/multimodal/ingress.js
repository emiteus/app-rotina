...
function wantsLayoutReproduce(mediaMeta, userText) {
  ...
  const t = `${userText || ''} ${mediaMeta.caption || ''}`;
  return looksLikeLayoutReproduceMessage(t);
}
...
async function tryLayoutImageEdit({ mediaMeta, mediaB64, userAsk, layoutObj }) {
  ...
  const { detectImageSize, nearestAspectRatio } = require('./creative');
  ...
  const img = await generateImage(prompt, {
    ...
    // pede frame maior quando o modelo aceitar (senão cai sem imageSize)
