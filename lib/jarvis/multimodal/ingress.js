...
async function tryLayoutImageEdit({ mediaMeta, mediaB64, userAsk, layoutObj }) {
  ...
  // Escudos oficiais (Wikimedia) — não deixa o modelo inventar
  let crestAssets = [];
  try {
    const { resolveCrestAssets } = require('./crests');
    const resolved = await resolveCrestAssets(userAsk);
    crestAssets = resolved.assets || [];
    if (resolved.swap?.unresolved) {
      console.log(
        JSON.stringify({
          tag: 'jarvis.crest',
          event: 'unresolved',
          raw: resolved.swap.raw
        })
      );
    }
  } catch (e) {
