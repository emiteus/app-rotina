...
async function tryLayoutImageEdit({ mediaMeta, mediaB64, userAsk, layoutObj }) {
  ...
  // Assets oficiais via Commons search (genérico — sem catálogo de times)
  let crestAssets = [];
  try {
  ...
  // 1) Compositor pixel (preferido): cola assets nos slots — logos reais, sem redesenhar
  if (crestAssets.length >= 2) {
    try {
      ...
      if (composed && composed.base64) {
        ...
        return {
          message: `Pronto — mesma estrutura (compose).\n${cap}`,
          multimodal: true,
          kind: mediaMeta.kind,
          provider: 'compose',
          visionV2: true,
  ...
  // 2) Fallback: modelo de imagem + assets anexados
