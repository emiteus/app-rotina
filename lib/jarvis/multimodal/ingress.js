  ...
  if (kind === 'audio' && modelList.length > 1) {
    ...
    const once = (model) => async (signal) => {
      ...
      const resp = await axios.post(
        ...
        { timeout: 20000, signal, headers: { 'x-goog-api-key': process.env.GEMINI_API_KEY } }
      );
    ...
    try {
      const lite = modelList[0];
      const race = [lite, lite, modelList.find((m) => m !== lite)].filter(Boolean);
      const text = await hedge(race.map(once), {
        delayMs: 2000,
        onHedge: () => console.log(JSON.stringify({ tag: 'jarvis.multimodal', event: 'stt_hedge', to: modelList[1] }))
      });
      if (text) return text;
    } catch (err) {
    ...
    // Os dois falharam: último recurso no laço normal com o que sobrou
    modelList.splice(0, 2);
  }
