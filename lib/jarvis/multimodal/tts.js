...
/** OpenAI sem crédito/limite → pula ela por 10 min (cada tentativa custava latência). */
...
async function openaiSpeech(speech, voice, timeoutMs = 45000) {
  ...
  if (!resp.ok) {
    ...
    if (resp.status === 429 || resp.status === 401 || /insufficient_quota|credit/i.test(errBody)) {
      openaiDownUntil = Date.now() + OPENAI_COOLDOWN_MS;
    }
...
async function geminiSpeech(speech, voice, timeoutMs = 45000) {
...
async function synthesizeSpeech(text, opts = {}) {
  ...
  if (allow('openai') && openaiKey() && Date.now() >= openaiDownUntil) {
    attempts.push(() => openaiSpeech(speech, opts.voice || process.env.JARVIS_TTS_VOICE || 'nova', left()));
  }
  if (allow('gemini') && geminiKey()) {
    attempts.push(() =>
