...
function ttsReady() {
  ...
  return !!(openaiKey() || geminiKey() || require('./fish-tts').fishKey());
}
...
async function synthesizeSpeech(text, opts = {}) {
  ...
  const fish = require('./fish-tts');
  const byProvider = {
    fish: allow('fish') && fish.fishKey() ? () => fish.fishSpeech(speech, { timeoutMs: left() }) : null,
    openai:
  ...
  // Com FISH_API_KEY a voz é SÓ a do Fish (Jarvis dublado): sem reserva de outra voz — trocar de voz
  // no meio da conversa era o problema (25/09/2026). Falhou = o texto aparece e ele não fala.
  // Sem Fish: Charon (Gemini), OpenAI de reserva. JARVIS_TTS_ORDER sobrepõe.
  const padrao = fish.fishKey() ? 'fish' : 'gemini,openai';
  const order = String(process.env.JARVIS_TTS_ORDER || padrao)
    .split(',')
    .map((x) => x.trim())
    .filter((x) => byProvider[x]);
  const attempts = [...new Set(order)].map((p) => byProvider[p]);

  // Cache: mesma frase + mesma voz = sai do banco, na hora e sem custo
  const cache = require('./tts-cache');
  const primeiro = [...new Set(order)][0];
  const variante = primeiro === 'fish' ? `fish|${JSON.stringify(fish.fishConfig())}` : null;
  if (variante) {
    const hit = await cache.getCached(speech, variante);
    if (hit) {
      console.log(JSON.stringify({ tag: 'jarvis.tts', event: 'cache', provider: 'fish', chars: speech.length }));
      return { base64: hit.buf.toString('base64'), mime: hit.mime, chars: speech.length, voice: 'jarvis', provider: 'fish', cached: true };
    }
  }
  ...
  for (const attempt of attempts) {
    ...
    try {
      ...
      if (out && variante && out.provider === 'fish') cache.putCached(speech, variante, out.buf, out.mime);
      if (!out) continue;
      console.log(
        JSON.stringify({
          tag: 'jarvis.tts',
