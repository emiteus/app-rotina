  ...
  for (const model of imageModels()) {
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      try {
        ...
        for (const ex of extras.slice(0, 4)) {
          parts.push({
            inlineData: {
...
function buildLockedReproducePrompt(layoutObj, userRequest, opts = {}) {
  ...
  if (crestAssets.length >= 2) {
    ...
    crestBlock = `
OFFICIAL CREST ASSETS (attached AFTER the reference template):
- Image #2 = OFFICIAL ${left.name || 'LEFT'} crest → PASTE EXACTLY into the LEFT crest slot.
- Image #3 = OFFICIAL ${right.name || 'RIGHT'} crest → PASTE EXACTLY into the RIGHT crest slot.
- Do NOT redraw, invent, distort, or "AI-approximate" club logos. PASTE the provided crest pixels.
