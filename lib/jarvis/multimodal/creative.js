...
function buildLockedReproducePrompt(layoutObj, userRequest, opts = {}) {
  ...
  if (crestAssets.length >= 2) {
    ...
    crestBlock = `
OFFICIAL CREST ASSETS (labeled images attached after the template):
- The image labeled LEFT crest = ${left.name} → PASTE EXACTLY into the LEFT crest slot of the template.
- The image labeled RIGHT crest = ${right.name} → PASTE EXACTLY into the RIGHT crest slot of the template.
- These are official club crest files. Copy their pixels. Do NOT redraw CRF/letters/stars from memory.
- Especially for Flamengo: keep the real interlocking white CRF monogram from the asset — never invent scribbles.
- You may uniformly scale and add the template's thick white outline around the pasted crest.
- Hallucinated/bootleg crests = FAIL.
`;
...
CRESTS / LOGOS (critical):
- Both crests MUST be the SAME visual HEIGHT.
- Horizontally ALIGN crest centers on the same baseline.
- Apply the template's thick white outer stroke to pasted crests (same weight as the reference crests).
- Never leave one crest smaller or without outline.
...
APPLY ONLY THE USER CHANGE (crest swap). Nothing else.

FORBIDDEN:
- VS / El Clasico / stadium / cinematic redesign
- Inventing club crests from memory when official assets were provided
