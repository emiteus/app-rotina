...
function validatePairSlots(left, right, imgW, imgH) {
  ...
  const maxSide = Math.round(Math.min(imgW, imgH) * 0.32);
  const minSide = Math.round(Math.min(imgW, imgH) * 0.1);
  const mid = imgW / 2;
  const gap = Math.round(imgW * 0.06);
...
async function detectSlotsByPixels(sharp, templateBuf, imgW, imgH) {
  ...
  const excludeL = Math.floor(tw * 0.4);
  const excludeR = Math.floor(tw * 0.6);
...
async function composeLayoutReplace(opts = {}) {
  ...
  // Vision também pode devolver caixas absurdas — valida
  if (slots.length >= 2) {
