...
function scoreCommonsTitle(title, query) {
  ...
  if (/\b(flag|kit|jersey|shirt|uniform|heart|dorgas|map|stadium|papacy|pope|vatican|panorama|aerial)\b/.test(t)) {
    score -= 50;
  }
  // Raster PNG > SVG (precisa thumb) > JPEG foto
  if (/\.png$/i.test(title)) score += 18;
  else if (/\.svg$/i.test(title)) score += 4;
  else if (/\.jpe?g$/i.test(title)) score -= 12;
  return score;
}
...
async function fetchCommonsFile(entry, { width = 640 } = {}) {
  ...
  try {
    ...
    try {
      ...
      /* fallback Special:FilePath — nunca para SVG (vira XML) */
    }
    if (!url) {
      if (isSvg) {
        console.log(
    ...
    if (/svg/i.test(ct) || (isSvg && !/png|jpeg|jpg|webp/i.test(ct))) {
      console.log(
        JSON.stringify({
          tag: 'jarvis.asset',
          event: 'svg_raw_skip',
    ...
    if (/jpeg|jpg/i.test(ct) || /\.jpe?g$/i.test(entry.file)) mime = 'image/jpeg';
    else if (/webp/i.test(ct)) mime = 'image/webp';
    else if (/png/i.test(ct)) mime = 'image/png';

    // Rejeita foto panorâmica / banner (não é crest)
    try {
      const sharp = (() => {
        try {
