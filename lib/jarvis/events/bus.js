...
async function notifyWarnNotes(notes, { source = 'sweep', userId = null } = {}) {
  ...
  if (userId && process.env.JARVIS_VOICE_ALERTS !== '0') {
    ...
    // Longe do PC: notificação no celular (página do Jarvis com aviso ligado) no lugar do WhatsApp
    try {
      const body = warns.map(noteSpeech).join(' ');
      const p = await require('../devices/phone-push').pushToPhones(userId, { title: 'Jarvis', body, tag: source });
      if (p.sent) {
        emit('proactive.notify', { channel: 'phone', source, count: warns.length });
        return { ok: true, channel: 'phone', count: warns.length, text, types: warns.map((w) => w.type) };
      }
    } catch (e) {
      console.error('[proactive] aviso no celular:', e.message);
    }
  }
...
/** Cron 5min — VPS/WhatsApp/CineRush no ar? (aviso por voz no PC ou celular: o WhatsApp pode ser o que caiu) */
async function runUptimeWatchNotify(userId) {
  if (process.env.JARVIS_UPTIME_WATCH === '0') {
    return { skipped: true, reason: 'disabled' };
  }
  const notes = await require('../ops/uptime-watch').checkUptime();
  if (!notes.length) return { skipped: true, reason: 'all_up', count: 0 };
  return notifyWarnNotes(notes, { source: 'uptime', userId });
}

/** Cron 1 min — publica os agendados do TeusHub e avisa o que saiu/falhou (socialhub-clock). */
async function runSocialhubPublishNotify(userId) {
  const r = await require('../ops/socialhub-clock').publishTick();
  if (!r.notes || !r.notes.length) return r;
  return notifyWarnNotes(r.notes, { source: 'socialhub', userId });
}

/** Cron diário — renova logins das redes do TeusHub e avisa login vencido. */
async function runSocialhubDailyNotify(userId) {
  const r = await require('../ops/socialhub-clock').dailyTokens();
  if (!r.notes || !r.notes.length) return r;
  return notifyWarnNotes(r.notes, { source: 'socialhub', userId });
}
...
module.exports = {
  ...
  runOpsWatchNotify,
  runUptimeWatchNotify,
  runSocialhubPublishNotify,
  runSocialhubDailyNotify,
