...
/** Alias/id as whole token(s) inside haystack — never raw substring (hub⊂teuhub). */
function hasPhraseToken(haystack, phrase) {
...
function resolveProject(query) {
  ...
  // Partial: only whole-token / whole-phrase matches (min 3 chars).
  // Avoids alias "hub" matching inside "teuhub" / "teushub".
  if (q.length >= 3) {
    let best = null;
    let bestLen = 0;
    for (const p of PROJECT_CATALOG) {
      const terms = [p.id, p.name, ...p.aliases].map(norm).filter((t) => t.length >= 3);
      for (const t of terms) {
        if (!hasPhraseToken(q, t) && !hasPhraseToken(t, q)) continue;
        if (t.length > bestLen) {
          best = p;
          bestLen = t.length;
        }
...
function resolveProjectsFromMessage(mensagem) {
  ...
  if (
    ...
    /\b(video|vídeo|reel|post)/i.test(mensagem) &&
    /\b(hoje|hj)\b/i.test(mensagem) &&
    !/\b(teushub|socialhub|social\s*hub|attracione|cinerush|cine\s*rush|erik|competi)\b/i.test(
      mensagem
    )
  ) {
    hits.push('attracione', 'cinerush_editor');
  }
  ...
  if (
    /\b(socialhub|social\s*hub|teuhub|teu\s*hub|teushub|teus\s*hub)\b/i.test(mensagem) ||
    (/\b(agendar\s+post|publicar\s+agendad)/i.test(mensagem) && !/\berik\b|\battracione\b|\bcompeti/i.test(mensagem))
  ) {
    hits.push('socialhub');
  }
  if (
    /\b(minerador|garimp\w*)\b/i.test(mensagem) ||
    (/\b(bombando|em\s*alta|viralizando|estourando)\b/i.test(mensagem) && /\bkwai\b/i.test(mensagem))
  ) {
    hits.push('minerador');
  }
  ...
  if (out.includes('attracione') && out.includes('socialhub')) {
    if (!/\b(socialhub|teuhub|teu\s*hub|teushub|teus\s*hub|agendar)\b/i.test(mensagem)) {
