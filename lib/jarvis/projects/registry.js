...
function resolveProjectsFromMessage(mensagem) {
  ...
  if (/\b(provision|chatwoot|havok|kirvano|cinerush|cine\s*rush|cinehub|cine\s*hub)\b/.test(mensagem)) {
    hits.push('cinerush');
  }
  if (/\b(attracione|attra)\b/.test(mensagem) || /\bcomp\s*\d+\b/i.test(mensagem)) {
    hits.push('attracione');
  }
  // NL: "views / vídeos / erik / filmes" → Attracione
  if (
    /\b(erik)\b/i.test(mensagem) ||
    (/\b(views?|visualiz)\b/i.test(mensagem) &&
      /\b(video|vídeo|corte|filme|tiktok|kwai|competi)/i.test(mensagem)) ||
