...
function hydrate(row) {
  ...
  const notas = Array.isArray(row.notas) ? row.notas : JSON.parse(row.notas || '[]');
...
async function upsertProjectMemory(userId, patch = {}) {
  ...
  if (patch.nota) {
    ...
    if (text) {
      notas = notas.filter((n) => String(n).toLowerCase() !== text.toLowerCase());
      notas.unshift(text);
...
function slimMemory(mem) {
  ...
  if (mem.decisoes?.length) {
    out.decisoes = mem.decisoes.slice(0, 5).map((d) => d.text || d);
  }
  if (mem.ultima_falha?.text) out.ultima_falha = mem.ultima_falha.text;
  if (mem.notas?.length) out.notas = mem.notas.slice(0, 8);
  return out;
}
