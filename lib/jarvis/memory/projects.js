...
async function upsertProjectMemory(userId, patch = {}) {
  ...
  const saved = await getProjectMemory(userId, projectId);
  // best-effort embed index — fora do caminho do turno (era await e segurava a resposta)
  const logReindexFail = (e) =>
    console.log(
      JSON.stringify({
        tag: 'jarvis.embed',
        event: 'reindex_fail',
  ...
  try {
    const { reindexProjectMemory } = require('./embed-index');
    Promise.resolve(reindexProjectMemory(userId, saved)).catch(logReindexFail);
  } catch (e) {
    logReindexFail(e);
  }
  return saved;
...
/**
 * Recall semântico — embeddings (Gemini) quando disponíveis; senão lexical.
 */
async function recallSemanticForMessage(
  ...
  // 1) Embeddings (se GEMINI_API_KEY + JARVIS_EMBEDDINGS≠0)
  try {
    const { embeddingsEnabled } = require('./embeddings');
    const { searchEmbeddings } = require('./embed-index');
    if (embeddingsEnabled()) {
      const projectIds = resolveProjectsFromMessage(mensagem);
      const embHits = await searchEmbeddings(userId, mensagem, {
      ...
      if (embHits.length) {
        return {
          mode: 'embedding',
          model: require('./embeddings').embeddingModel(),
          hits: embHits,
          empty: false
        };
      }
      // sem hits emb — ainda tenta lexical abaixo
    }
  } catch (e) {
    console.log(
      JSON.stringify({
