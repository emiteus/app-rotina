...
const TOOL_DEFS = {
  ...
  criar_recorrente: {
    ...
    timeoutMs: 10000,
    description:
      'Cria tarefa que se REPETE ("todo dia faço prancha ao acordar", "toda segunda e quarta academia"). Args: titulo; dias_semana? = lista (seg, ter, qua, qui, sex, sab, dom) — sem dias = todo dia.'
  },
  criar_meta: {
    risk: 'medium',
    timeoutMs: 10000,
  ...
  concluir_tarefa: {
    risk: 'low',
    timeoutMs: 10000,
    description: 'Marca tarefa como FEITA (só quando ele fez). Pra tirar/apagar/remover duplicada → apagar_tarefa, NUNCA concluir.'
  },
  apagar_tarefa: {
    risk: 'medium',
    timeoutMs: 10000,
    description:
      'Apaga tarefa do dia (não conclui). Args: titulo ou id, data_reset? (YYYY-MM-DD, padrão hoje); duplicadas: true = apaga só as repetidas do dia (mantém 1 de cada, a concluída se houver).'
  },
  criar_evento: {
    risk: 'low',
    timeoutMs: 10000,
  ...
  socialhub_posts: {
    ...
    ownerOnly: true,
    description:
      'Lista posts TeusHub/SocialHub (com o motivo de cada falha). Contagem de hoje → preferir projetos.socialhub.hoje no snapshot. Args: status agendados|publicados|falhos (ou SCHEDULED|PUBLISHED|FAILED), limit, id.'
  },
  socialhub_agendar: {
    risk: 'high',
    timeoutMs: 30000,
    ...
    mutatesProjetos: true,
    description:
      'Agenda post no TeusHub. Args: caption, contas (["instagram","tiktok"] | "todas" | ids), scheduledAt ISO (sem = daqui 1 min), media_url (Instagram precisa de mídia; TikTok de vídeo; YouTube não agenda por aqui).'
  },
  teushub_ney_filmes: {
    risk: 'high',
    timeoutMs: 45000,
    ...
    mutatesProjetos: true,
    description:
      'Ney Filmes: pega os vídeos de uma PASTA DO PC (cada arquivo = nome do filme/série), acha a sinopse de cada obra e agenda no Instagram da categoria Ney Filmes do TeusHub (até 15/dia, horários variados; legenda = sinopse + "🎬 Tipo: Nome"). Só essa categoria. Args: pasta (ex.: "Downloads/Ney", "Área de Trabalho/cortes").'
  },
  socialhub_post_acao: {
    risk: 'high',
    timeoutMs: 60000,
    ...
    mutatesProjetos: true,
    description:
      'Mexe em post do TeusHub. Args: acao cancelar|reagendar|tentar_novamente; id ou ids[] (sem id = todos os agendados, ou todos os falhos em tentar_novamente); scheduledAt ISO (início); intervalo_min + variacao_min pra espalhar vários ("um a cada 10-15 min" = intervalo_min 10, variacao_min 5).'
  },
  ...
  // —— Minerador (braço: cortes em alta no Kwai; R:\Projetos\Minerador) ——
  minerador_em_alta: {
    risk: 'low',
    timeoutMs: 20000,
    ownerOnly: true,
    description:
      'Cortes de filme no Kwai (Minerador). Args: ordem "alta" (crescendo agora, padrão) | "views" (os mais vistos — use em "mais virais", "mais vistos"), min_views ("100k", 100000), dias (quantos dias pra trás; padrão 7, com ordem views 30), limite (padrão 10), pagina?. "o que tá bombando no Kwai", "o que cortar hoje", "os mais virais com mais de 100k".'
  },
  minerador_varrer: {
    risk: 'low',
    timeoutMs: 60000,
    ownerOnly: true,
    description:
      'Analisa TODOS os vídeos de UMA página do Kwai (não só os recentes) e devolve os mais virais. Args: pagina (@ ou link), min_views ("100k"), limite (padrão 10), dias? (só os dos últimos N dias). Página grande (milhares de vídeos) leva minutos: segue em segundo plano e avisa no fim. "Pega os mais virais da página X", "analisa todos os vídeos da @Y". Não cadastra a página (isso é minerador_pagina).'
  },
  minerador_paginas: {
    risk: 'low',
    timeoutMs: 15000,
    ownerOnly: true,
    description: 'Lista as páginas do Kwai que o Minerador vigia (com erro de cada uma e a última rodada).'
  },
  minerador_pagina: {
    risk: 'medium',
    timeoutMs: 60000,
    ownerOnly: true,
    description: 'Adiciona ou para de vigiar página(s) do Kwai no Minerador. Args: acao adicionar|remover, paginas (lista de @ ou links — link curto do app k.kwai.com/p/… vale; pode colar vários). Página parada há +14 dias não entra.'
  },
  minerador_video: {
    risk: 'low',
    timeoutMs: 15000,
    ownerOnly: true,
    description:
      'Marca vídeo do Minerador: status descartado|cortado|novo. Args: videoId. "Corta o 1º" = cinerush_editor_process com a url dele + minerador_video status cortado.'
  },
  cinerush_editor_process: {
    ...
    mutatesProjetos: true,
    description:
      'Enfileira 1 corte no CineRush Editor (massa IG) — NÃO é CineRush TV. Args: url (Kwai/YouTube/TikTok), manual_headline?, clip_duration?.'
  },
  ...
  cinerush_editor_jobs: {
    ...
    ownerOnly: true,
    description:
      'CineRush Editor: trabalhos recentes (processando, na fila, com ERRO) e de quem são. "Tem erro processando vídeo?", "o que o Erik está processando/reprocessando?", "como tá a fila?". Args: status? = erro | processando | fila | todos; usuario? (parte do e-mail ou nome, ex. erik); pergunta = o que ele quer saber.'
  },
  cinerush_editor_agendados: {
    ...
    ownerOnly: true,
    description:
      'CineRush Editor: posts no Instagram AGENDADOS (programados), que FALHARAM ou já POSTADOS, por usuário. "Analisa meus posts programados", "quais posts falharam?", "o Erik tem posts agendados?". Args: status? = agendados | falhos | postados; usuario?; pergunta = o que ele quer saber.'
  },
  ...
  pc_open_app: {
    ...
    ownerOnly: true,
    description:
      'Abre app da lista liberada no PC. Args: app = spotify | chrome | vscode | explorer | notepad | calculadora | whatsapp | discord | obs | terminal | cursor | opera (Opera GX) | steam | cs2 (Counter-Strike 2, abre pela Steam).'
  },
  pc_close_app: {
    risk: 'high',
    timeoutMs: 20000,
  ...
  pc_download: {
    ...
    ownerOnly: true,
    description:
      'Baixa vídeo/música/arquivo NO PC (Baixador de Vídeos: Kwai, TikTok, Instagram, YouTube, Facebook e ~1800 sites; pdf/zip/imagem direto). Args: urls (lista de links, até 20; pode colar texto com link), formato mp4|mp3 (padrão mp4), nome? (nome do arquivo — ex.: nome do filme pro Ney Filmes), pasta? (padrão Downloads/Jarvis). Executável não baixa. "baixa esses vídeos", "baixa o 1º e o 2º do em alta".'
  },
  pc_close_tab: {
    risk: 'medium',
    timeoutMs: 30000,
    ownerOnly: true,
    description:
      'Fecha UMA aba do navegador do PC pelo nome ("fecha a aba do YouTube" → aba = YouTube). Passa pelas abas até achar a que tem esse texto no título. "Fecha essa aba" (sem nome) → aba = "essa" (fecha a da frente). Args: aba (texto do título, curto).'
  },
  pc_screen_look: {
    risk: 'medium',
    timeoutMs: 60000,
  ...
  pc_spotify_play: {
    ...
    ownerOnly: true,
    description:
      'Toca no Spotify do usuário (Premium). Args: busca (artista, música, álbum ou playlist, ex. "Matuê", "Kenny G Matuê"); tipo_busca? = artista | musica | album | playlist | curtidas (as músicas que ELE curtiu / "minhas favoritas" / Liked Songs, sem busca) | mais_ouvidas (as mais tocadas DELE, sem busca).'
  },
  pc_spotify_now: {
    risk: 'low',
    timeoutMs: 20000,
