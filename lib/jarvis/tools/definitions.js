...
const TOOL_DEFS = {
  ...
  dev_propose_patch: {
    ...
    ownerOnly: true,
    description:
      'Propõe patch (preview diff, NÃO grava). Args: project, path, content (arquivo completo). Depois apply_local ou github_pr com SIM.'
  },
  dev_apply_patch_local: {
    ...
    ownerOnly: true,
    description:
      'Grava patch no disco local (HITL). Args: project, path, content. Precisa PROJETOS_ROOT. Sem commit automático.'
  },
  dev_github_pr: {
    risk: 'critical',
    timeoutMs: 45000,
    ownerOnly: true,
    description:
      'Abre PR no GitHub com 1 arquivo (HITL). Args: project, path, content, title?, base?. Precisa GITHUB_TOKEN (repo).'
  },
  dev_railway_logs: {
    risk: 'low',
    timeoutMs: 25000,
