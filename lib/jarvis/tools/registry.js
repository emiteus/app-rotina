...
/**
 * Bloco compacto pro system prompt — fonte única (TOOL_DEFS), não duplicar no ia.js.
 * Emite schema mínimo: {"tipo":"nome", ...args sugeridos no description}.
 */
function toolsPromptBlock() {
  const lines = listToolNames().map((name) => {
    const t = TOOL_DEFS[name];
    return `- {"tipo":"${name}"} — ${t.description}`;
  });
  return lines.join('\n');
}
