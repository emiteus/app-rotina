...
const pc = require('./handlers/pc');
const tv = require('./handlers/tv');
...
async function executarAcoesCorpo(acoes, userId) {
  ...
  for (const acao of acoes.slice(0, 8)) {
    ...
    try {
      ...
      if (!r) r = await browser.handle(acao, ctx);
      if (!r) r = await pc.handle(acao, ctx);
      if (!r) r = await tv.handle(acao, ctx);
      if (!r) r = await social.handle(acao, ctx);
