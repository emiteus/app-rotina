...
/**
 * Ação que nada na mensagem pediu (25/09: "tenta de novo os posts que falharam no TeusHub" veio com
 * pc_close_tab {aba: "essa"} de brinde e o PC tentou fechar uma aba). Só vale com a mensagem na mão.
 */
const NEEDS_WORD = {
  pc_close_tab: /\b(abas?|guias?|tabs?)\b/i
};
function offTopic(acao, mensagem) {
  const re = NEEDS_WORD[acao && acao.tipo];
  if (!re || !mensagem) return false;
  return !re.test(String(mensagem));
}

/** "tenta de novo", "repete", "outra vez" — pedido explícito de repetir o que já foi feito. */
function wantsRepeat(mensagem) {
  return /\b(de novo|denovo|novamente|outra vez|repete|repetir|refaz|refazer|mais uma vez)\b/i.test(String(mensagem || ''));
}
...
async function runToolBatch({
  ...
  const sliced = acoes
    ...
    .filter(Boolean)
    .filter((a) => {
      if (offTopic(a, mensagem)) {
        console.log(JSON.stringify({ tag: 'jarvis.tool', event: 'off_topic_dropped', tipo: a.tipo, userId }));
        return false;
      }
      return true;
    });

  // Tipo fora do registro não tem risco/owner conhecidos → bloqueia (antes caía como
  // "medium, não-owner, sem SIM" e só não rodava porque nenhum handler aceitava).
  const unregisteredBlocked = [];
  const registered = [];
  for (const a of sliced) {
    if (resolveToolMeta(a && a.tipo).registered) registered.push(a);
    else {
      unregisteredBlocked.push({
        tipo: (a && a.tipo) || 'desconhecido',
        ok: false,
        erro: 'tool não registrada',
        unregistered: true
      });
    }
  }
  ...
  // Tools rodam em sequência → o lote precisa da soma, não do maior (senão corta no meio
  // e reporta falha de algo que ainda está rodando). Teto de 5 min por turno.
  const batchTimeout = Math.min(
    Math.max(30000, metas.reduce((sum, m) => sum + (m.timeoutMs || 30000), 0)),
    300000
  );
  ...
  if (toRun.length) {
    try {
      ...
      // Timeout do lote: as tools podem ter rodado — não afirmar falha
      const timedOut = /timeout/i.test(String(err.message || ''));
      feitos = toRun.map((a) => ({
        tipo: a.tipo,
        ok: false,
        uncertain: timedOut || undefined,
        erro: timedOut
          ? `sem confirmação (${err.message}) — pode ter rodado; confere antes de repetir`
          : err.message || 'tool-batch falhou'
      }));
    }
  }
  ...
  // Mesma ação que você aprovou há pouco (o modelo re-emite ao ler o histórico, ex. "Quando os posts
  // forem postados, me avise" pedia de novo pra repostar) → não pergunta de novo. "De novo/repete" libera.
  let toHold = gated;
  if (gated.length && !wantsRepeat(mensagem)) {
    try {
      const { recentlyApprovedSignatures, acaoSignature } = require('../permissions/approvals');
      const recent = await recentlyApprovedSignatures(userId);
      if (recent.size) {
        toHold = [];
        for (const a of gated) {
          const at = recent.get(acaoSignature(a));
          if (!at) {
            toHold.push(a);
            continue;
          }
          const min = Math.max(1, Math.round((Date.now() - new Date(at).getTime()) / 60000));
          results.push({
            tipo: a.tipo,
            ok: false,
            already_done: true,
            erro: `já fiz isso há ${min} min (você aprovou) — não repeti. Se quiser de novo, diz "repete"`
          });
        }
      }
    } catch (e) {
      console.error('[jarvis] dedupe aprovação:', e.message);
    }
  }
  if (toHold.length) {
    const held = await holdForApproval(userId, toHold, { channel });
    results.push(...held);
  }
  ...
  // Fase 1: falha vira lição, sucesso resolve (feitos[i] é o resultado de toRun[i]).
  // Fora do caminho da resposta — não segura o turno.
  if (toRun.length && Array.isArray(feitos)) {
    const pairs = toRun
      .map((acao, i) => ({ acao, result: feitos[i] }))
      .filter((p) => p.result && p.result.tipo === p.acao.tipo);
    require('../learning/lessons')
      .recordOutcomes(userId, pairs, { projectIdFromTool })
      .catch((e) => console.error('[jarvis.lesson] record:', e.message));
  }
