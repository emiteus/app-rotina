...
async function handleGenerateImage(acao) {
  ...
  try {
    ...
    const { looksLikeLayoutTweakMessage, looksLikeLayoutReproduceMessage } = require('../../multimodal/ingress');
    const isLayoutEdit =
      !!(acao.locked_layout || acao.lockedLayout) ||
      looksLikeLayoutTweakMessage(prompt) ||
      looksLikeLayoutReproduceMessage(prompt);
    if (isLayoutEdit && !refB64) {
      return {
        tipo: 'creative_generate_image',
        ok: false,
        erro:
          'Ajuste de layout precisa da imagem de referência. Manda o print de novo ou responde à imagem anterior.'
      };
    }
    const locked = !!(acao.locked_layout || acao.lockedLayout || refB64);
