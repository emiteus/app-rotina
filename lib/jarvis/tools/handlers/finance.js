...
const { requireLib, requireRoutes } = require('../../host');
const { all, run, get } = requireLib('db');
const { hojeStr } = requireLib('datas');
const plano = requireLib('plano-financeiro');
const openfinanceRouter = requireRoutes('openfinance');
...
async function handle(acao, ctx) {
  ...
  if (tipo === 'recategorizar') {
    ...
    if (!txs.length) {
      ...
      if (contem.length && openfinanceRouter.temCredenciais && openfinanceRouter.temCredenciais()) {
        try {
          ...
          syncInfo = { erro: String((syncErr && syncErr.message) || syncErr).slice(0, 160) };
          console.warn(
            JSON.stringify({ tag: 'jarvis.finance', event: 'sync_failed', erro: syncInfo.erro })
          );
        }
      }
    }
    if (!txs.length) {
      ...
      const syncTxt = syncInfo && syncInfo.erro
        ? ` ⚠️ A sincronização com o banco falhou (${syncInfo.erro}) — pode ser só isso.`
        : syncInfo && Number(syncInfo.importadas) > 0
    ...
    if (deRaw) {
      ...
      if (!row) {
        row = await get(
          ...
          [likeContains(deRaw), likeContains(chaveTry), userId]
        );
      }
    }
    if (!row) {
    ...
    if (!row && titulo) {
      row = await get(
        `SELECT id, titulo, status FROM despesas_mes
         ...
         LIMIT 1`,
        [userId, acao.ym || ym, titulo, likeContains(titulo)]
      );
    }
    if (!row) {
    ...
    if (!row && titulo) {
      row = await get(
        `SELECT * FROM receitas_mes
         ...
         LIMIT 1`,
        [userId, ymRec, titulo, likeContains(titulo)]
      );
    }
    if (!row) {
  ...
  if (tipo === 'reconciliar_despesas') {
    const { reconciliarMes } = requireRoutes('despesas');
    const ymRec = String(acao.ym || ym).slice(0, 7);
  ...
  if (tipo === 'sincronizar_bancos') {
    const openfinanceLocal = requireRoutes('openfinance');
    const { reconciliarMes } = requireRoutes('despesas');
