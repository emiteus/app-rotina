  ...
  if (
    ...
    if (tipo === 'cinerush_criar') {
      return {
        ...
        ok: false,
        erro:
          'Criar assinante novo **não está no hub** — cadastro entra só via venda Kirvano. ' +
          'Depois que estiver **pendente**, use cinerush_provisionar pra liberar o acesso.'
      };
    }
        ...
        if (!hit) {
          return {
            ...
            erro:
              ...
              `Provisionar só libera quem já está na fila (pendente) — criar cadastro novo ainda não está no hub.`
          };
        }
        id = hit.id;
      }
