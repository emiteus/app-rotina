...
async function mineradorAcao(mn, tipo, acao) {
  if (tipo === 'minerador_em_alta') {
    const out = await mn.feed({ janela: acao.janela || 6, limite: Math.min(20, acao.limite || 10), pagina: acao.pagina });
    return {
      tipo,
      ok: true,
      janelaH: out.janelaH,
  ...
  if (tipo === 'minerador_pagina') {
    const ref = String(acao.pagina || acao.handle || acao.link || '').trim();
    if (!ref) return { tipo, ok: false, erro: 'Qual página? Manda o @ ou o link de um vídeo dela' };
    if (/^(remover|parar|tirar|desligar)/i.test(String(acao.acao || ''))) {
      await mn.removeSource(ref);
      return { tipo, ok: true, acao: 'remover', pagina: ref.replace(/^@/, '') };
    }
    const out = await mn.addSource(ref);
    return { tipo, ok: true, acao: 'adicionar', pagina: out.handle, videos: out.videos };
  }
...
async function handle(acao, ctx) {
  ...
  if (tipo.startsWith('minerador_')) {
    ...
    return mineradorAcao(mn, tipo, acao);
  }
