...
async function mineradorAcao(mn, tipo, acao) {
  if (tipo === 'minerador_em_alta') {
    // "100k", "100 mil", 100000 → número
    const minViews = (() => {
      const v = String(acao.min_views ?? acao.minimo ?? '').toLowerCase().replace(/\s+/g, '');
      const m = v.match(/^([\d.,]+)(k|mil|m|mi|milh[oõ]es)?$/);
      if (!m) return 0;
      // "100.000" = cem mil; "1.5k"/"1,5 mil" = mil e quinhentos
      const n = m[2] ? Number(m[1].replace(',', '.')) : Number(m[1].replace(/[.,]/g, ''));
      return Math.round(n * (m[2] === 'k' || m[2] === 'mil' ? 1e3 : m[2] ? 1e6 : 1));
    })();
