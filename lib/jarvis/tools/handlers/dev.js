...
function isPathUnderRoot(rootDir, candidate) {
  const r = path.resolve(rootDir).replace(/[/\\]+$/, '').toLowerCase();
  const c = path.resolve(candidate).toLowerCase();
  return c === r || c.startsWith(r + path.sep) || c.startsWith(r + '/') || c.startsWith(r + '\\');
}
...
function localProjectPath(key) {
  ...
  try {
    if (fs.existsSync(rootResolved)) rootResolved = fs.realpathSync(rootResolved);
  } catch (_) {
    /* keep resolve */
  }
  const p = path.resolve(rootResolved, meta.localFolder);
  if (!isPathUnderRoot(rootResolved, p)) return null;
  return p;
}
...
function assertSafeRelPath(rel) {
  ...
  if (!s || s.includes('..') || path.isAbsolute(rel) || s.includes('\0')) {
...
/**
 * Resolve path under project root following symlinks — bloqueia escape via link.
 */
function resolvePathUnderProject(projectRoot, relPath) {
  const rel = assertSafeRelPath(relPath);
  let rootReal = path.resolve(projectRoot);
  try {
    if (fs.existsSync(rootReal)) rootReal = fs.realpathSync(rootReal);
  } catch (_) {
    /* keep */
  }
  const joined = path.resolve(rootReal, rel);
  let realTarget = joined;
  try {
    if (fs.existsSync(joined)) {
      realTarget = fs.realpathSync(joined);
    } else {
      // arquivo novo: realpath do dir pai (criado sob o root)
      ...
      let dirReal = dir;
      try {
        if (fs.existsSync(dir)) dirReal = fs.realpathSync(dir);
      } catch (_) {
        /* keep */
      }
      realTarget = path.join(dirReal, ...parts, path.basename(joined));
    }
  } catch (e) {
    throw new Error(`path inválido: ${rel} (${e.message})`);
  }
  const rootCmp = rootReal;
  if (!isPathUnderRoot(rootCmp, realTarget)) {
    throw new Error(`path fora do projeto (symlink?): ${rel}`);
  }
  return realTarget;
}

/** Redige tokens/secrets antes de mandar logs pro WA. */
function redactSecrets(text) {
  return String(text || '')
    ...
    .replace(/\b(ghp_|gho_|ghu_|ghs_|github_pat_|sk-|rk_|xox[baprs]-)[A-Za-z0-9_\-]+/g, '[redacted]')
    .replace(/\b(eyJ[A-Za-z0-9_-]{10,}\.[A-Za-z0-9_-]{10,}\.[A-Za-z0-9_-]+)/g, '[redacted-jwt]');
}
...
async function handleRailwayLogs(acao) {
  ...
  try {
    ...
    lines = (lines || []).map((ln) => redactSecrets(ln));
...
async function handleApplyPatchLocal(acao) {
  ...
  const written = [];
  try {
    // Marca antes de gravar — falha no meio ainda deixa arquivo novo no disco
    require('../../permissions/patch-marks').markLocalPatch(key);
    for (const f of files) {
      const full = resolvePathUnderProject(local, f.path);
      fs.mkdirSync(path.dirname(full), { recursive: true });
      // Re-checa depois do mkdir (symlink no meio do caminho)
      const full2 = resolvePathUnderProject(local, f.path);
      const created = !fs.existsSync(full2);
      fs.writeFileSync(full2, f.content, 'utf8');
...
async function handleRunTests(acao) {
  ...
  // Chegou aqui = auto (sem patch pendente) ou aprovado com SIM
  require('../../permissions/patch-marks').clearLocalPatch(key);
