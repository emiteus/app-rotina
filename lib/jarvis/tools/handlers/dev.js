...
async function handleDeployChecklist(acao) {
  ...
  const lines = [
    ...
    meta.railwayService
      ? `· Railway service: \`${meta.railwayService}\``
      : '· Sem Railway mapeado',
    '1. Código OK? `dev_git_status` / `dev_run_tests` (disco)',
    '2. Mudou Jarvis? `npm run sync:host` no repo jarvis-os + commit app-rotina',
    '3. Abriu PR? `dev_github_pr` (HITL SIM)',
    '4. Subir host? `dev_railway_redeploy` no **approtina** (HITL SIM) — ou restart se só env',
    '5. Smoke WA: mensagem curta + tool crítica com SIM',
    '',
