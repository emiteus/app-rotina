...
async function handleProposePatch(acao) {
  ...
  try {
    rel = assertSafeRelPath(acao.path || acao.file || acao.arquivo);
    pack = assertPatchContent(acao.content || acao.conteudo || acao.body);
  } catch (e) {
    return { tipo: 'dev_propose_patch', ok: false, erro: e.message };
  }

  try {
    const cur = await readProjectFileText(key, rel);
    if (cur.content === pack.content) {
      return {
        ...
        path: rel,
        unchanged: true,
        texto: `Patch *${key}/${rel}*: conteúdo idêntico — nada a aplicar.`
      };
    }
    const diff = unifiedDiff(rel, cur.content, pack.content);
    const beforeLines = cur.content ? cur.content.split(/\r?\n/).length : 0;
    const afterLines = pack.content.split(/\r?\n/).length;
    return {
      tipo: 'dev_propose_patch',
      ok: true,
      project: key,
      ...
      // echo content so apply/PR can reuse after HITL without re-asking LLM
      content: pack.content,
      texto:
        `*Patch proposto* \`${key}/${rel}\` (${beforeLines}→${afterLines} linhas, ${pack.bytes}B)\n` +
        `Fonte atual: ${cur.fonte || 'novo'}\n` +
        `Pra gravar: **dev_apply_patch_local** (disco) ou **dev_github_pr** (PR) — pedem SIM.\n` +
        `\`\`\`diff\n${diff.slice(0, 2200)}\n\`\`\``
    };
  } catch (e) {
    return { tipo: 'dev_propose_patch', ok: false, erro: e.message };
...
async function handleApplyPatchLocal(acao) {
  ...
  const full = path.resolve(local, rel);
  if (!full.startsWith(path.resolve(local))) {
    return { tipo: 'dev_apply_patch_local', ok: false, erro: 'path fora do projeto' };
  }

  try {
    fs.mkdirSync(path.dirname(full), { recursive: true });
    const before = fs.existsSync(full) ? fs.readFileSync(full, 'utf8') : '';
    fs.writeFileSync(full, pack.content, 'utf8');
    return {
      tipo: 'dev_apply_patch_local',
      ok: true,
      project: key,
      ...
      created: !before,
      texto:
        `*Patch aplicado* (local) \`${key}/${rel}\` (${pack.bytes}B). ` +
        `Não fiz commit — revisa e commit/PR manual ou manda **dev_github_pr**.`
    };
  } catch (e) {
    return { tipo: 'dev_apply_patch_local', ok: false, erro: e.message };
  }
...
async function handleGithubPr(acao) {
  ...
  const title =
    String(acao.title || acao.titulo || `Jarvis: update ${rel}`).trim().slice(0, 120) ||
    `Jarvis: update ${rel}`;
  const body = String(
    acao.body_pr ||
      acao.pr_body ||
  ...
  try {
    ...
    await githubFetch(
        ...
        .join('/')}`,
      {
        ...
        body: {
          message: title,
          content: Buffer.from(pack.content, 'utf8').toString('base64'),
          branch: branchName,
          ...(fileSha ? { sha: fileSha } : {})
        }
      }
    ...
    return {
      ...
      pr_number: pr.number,
      pr_url: pr.html_url,
      texto: `*PR aberto* [#${pr.number}](${pr.html_url})\n\`${meta.github}\` · \`${branchName}\` → \`${base}\` · \`${rel}\``
    };
  } catch (e) {
    return {
      tipo: 'dev_github_pr',
