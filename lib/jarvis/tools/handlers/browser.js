 ...
 */
const { safeFetch, stripHtml, decodeHtml } = require('./research');
...
function extractHeadings(html, limit = 12) {
  ...
  while ((m = re.exec(html)) && out.length < limit) {
    ...
    // SPA truncada: "Competição de Cortes da" — ignora heading cortado
    if (text.length < 3) continue;
    if (/\b(da|de|do|dos|das|e|ou)\s*$/i.test(text) && text.length < 40) continue;
...
async function handleOpen(acao, userId) {
  ...
  const wantHeadless =
    ...
    acao.render === true ||
    acao.js === true ||
    String(acao.mode || '').toLowerCase() === 'headless';

  if (wantHeadless) {
    try {
      const headless = require('../../browser/headless');
      const snap = await headless.navigate(userId, url);
      return {
        tipo: 'browser_open',
        ok: true,
        url: snap.url,
        ...
        text: snap.text,
        texto:
          `*Browser headless* \`${snap.url}\`` +
          (snap.title ? `\n**${snap.title}**` : '') +
          (snap.text ? `\n\`\`\`\n${String(snap.text).slice(0, 2200)}\n\`\`\`` : '')
      };
    } catch (e) {
      return {
        tipo: 'browser_open',
        ...
        hint:
          e.code === 'HEADLESS_OFF'
            ? 'JARVIS_BROWSER_HEADLESS=1 no Railway'
            : e.code === 'NO_PLAYWRIGHT'
              ? 'npm i playwright && npx playwright install chromium'
              : 'Allowlist RESEARCH_FETCH_*'
      };
    }
  }
...
async function handle(acao, ctx) {
  ...
  if (tipo === 'browser_open') return handleOpen(acao, ctx.userId);
  if (tipo === 'browser_links') return handleLinks(acao);

  const headless = require('../../browser/headless');
  try {
    if (tipo === 'browser_click') {
      const out = await headless.click(ctx.userId, {
        selector: acao.selector || acao.css,
        text: acao.text || acao.label
      });
      return {
        tipo,
        ok: true,
        ...out,
        texto: `Cliquei em ${acao.selector || acao.text || '?'} → \`${out.url}\``
      };
    }
    if (tipo === 'browser_type') {
      const out = await headless.type(ctx.userId, {
        selector: acao.selector || acao.css,
        text: acao.text ?? acao.value ?? '',
        clear: acao.clear !== false
      });
      return {
        tipo,
        ok: true,
        ...out,
        texto: `Digitei em \`${acao.selector}\``
      };
    }
    if (tipo === 'browser_snapshot') {
      const out = await headless.snapshot(ctx.userId, {
        maxChars: Number(acao.max_chars || 4000) || 4000
      });
      return {
        tipo,
        ok: true,
    ...
    return {
      ...
      hint:
        e.code === 'HEADLESS_OFF'
          ? 'JARVIS_BROWSER_HEADLESS=1'
          : e.code === 'NO_SESSION'
            ? 'browser_open com headless:true antes'
            : undefined
    };
  }
  return null;
