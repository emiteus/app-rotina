...
async function handleWriteReport(acao, ctx) {
  ...
  if (hits.length) {
    const withText = hits.filter((h) => String(h.snippet || '').trim());
    findings = (withText.length ? withText : hits).slice(0, 10).map((h) => {
      const sn = String(h.snippet || '').trim();
      return sn
        ? `${h.title || 'fonte'}: ${sn.slice(0, 180)}`
        : `${h.title || 'fonte'} → ${h.url || ''}`.trim();
    });
    sources = hits
      .filter((h) => h.url)
      .map((h) => ({ title: h.title || h.url, url: h.url }));
    body =
      `Com base em **${hits.length}** resultados de busca:\n` +
      hits
        .slice(0, 8)
        .map((h, i) => {
          const sn = String(h.snippet || '').trim().slice(0, 140);
          return sn
  ...
  if (sources.length) {
    ...
    for (const s of sources.slice(0, 15)) {
      if (typeof s === 'string') md += `- ${s}\n`;
        ...
        md += `- ${s.title || s.nome || s.url || s.link}: ${s.url || s.link}\n`;
      }
    }
    md += '\n';
  }
