...
async function handleWriteReport(acao, ctx) {
  ...
  const findings = Array.isArray(acao.findings) ? acao.findings : [];
  const sources = Array.isArray(acao.sources)
    ? acao.sources
...
async function handle(acao, ctx) {
  ...
  if (tipo === 'research_web_search') return handleWebSearch(acao);
