  ...
  for (const h of hits || []) {
    ...
    if (!hit.snippet && hit.url && fetched < maxFetch) {
      try {
        ...
        // redirect pode ter saído do host validado (ex.: → 127.0.0.1)
        if (res.url && res.url !== u.href) await assertSafeUrlAllowHit(res.url);
        if (res.ok) {
...
module.exports = { handle, TYPES, assertSafeUrl, stripHtml, decodeHtml, researchFetchOpenEnabled };
