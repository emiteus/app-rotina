const { v4: uuid } = require('uuid');
const { likeContains } = require('./helpers');
const { requireLib } = require('../../host');
const { run, get, all } = requireLib('db');
const { checkinHabito } = requireLib('habitos');
const { hojeStr, dataResetSql } = requireLib('datas');
const { persistirHistoricoDia } = requireLib('historico');

const TYPES = new Set([
  'criar_tarefa',
  'criar_recorrente',
  'marcar_habito',
  'concluir_tarefa',
  'apagar_tarefa',
  'criar_evento',
  'criar_alarme'
]);

async function handle(acao, ctx) {
  const tipo = acao && acao.tipo;
  if (!TYPES.has(tipo)) return null;

  const { userId } = ctx;

  // Tarefa que se repete ("todo dia faço prancha ao acordar", 22/09/2026 virava evento e falhava)
  if (tipo === 'criar_recorrente') {
    const titulo = String(acao.titulo || '').trim().slice(0, 120);
    if (!titulo) return { tipo, ok: false, erro: 'titulo obrigatório' };
    const DIAS = { dom: 0, domingo: 0, seg: 1, segunda: 1, ter: 2, terca: 2, 'terça': 2, qua: 3, quarta: 3, qui: 4, quinta: 4, sex: 5, sexta: 5, sab: 6, sabado: 6, 'sábado': 6 };
    let dias = '0,1,2,3,4,5,6';
    const pedido = acao.dias_semana || acao.dias;
    if (pedido) {
      const lista = (Array.isArray(pedido) ? pedido : String(pedido).split(/[,\s]+/))
        .map((d) => (/^\d$/.test(String(d)) ? Number(d) : DIAS[String(d).toLowerCase().normalize('NFC')]))
        .filter((d) => Number.isInteger(d) && d >= 0 && d <= 6);
      if (!lista.length) return { tipo, ok: false, erro: 'dias da semana inválidos' };
      dias = [...new Set(lista)].sort().join(',');
    }
    const dup = await get(
      `SELECT id FROM tarefas_recorrentes WHERE user_id = $1 AND lower(titulo) = lower($2) AND COALESCE(ativa, true) = true`,
      [userId, titulo]
    ).catch(() => null);
    if (dup) return { tipo, ok: true, titulo, ja: true, dias };
    await run(
      `INSERT INTO tarefas_recorrentes (id, titulo, descricao, prioridade, categoria, frequencia, dias_semana, user_id)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`,
      [uuid(), titulo, acao.descricao || '', acao.prioridade || 'media', acao.categoria || 'geral', 'diario', dias, userId] // 'diario' respeita dias_semana (cron do host)
    );
    return { tipo, ok: true, titulo, dias };
  }

  if (tipo === 'criar_tarefa') {
    const titulo = String(acao.titulo || '').trim();
    if (!titulo) {
      return { tipo, ok: false, erro: 'titulo obrigatório' };
    }
    const id = uuid();
    const dataReset = acao.data_reset && String(acao.data_reset).length >= 10
      ? dataResetSql(String(acao.data_reset).slice(0, 10))
      : dataResetSql(hojeStr());
    // Mesma tarefa no mesmo dia = não duplica (25/09: pedido repetido criou tudo 2x)
    const dup = await get(
      `SELECT id FROM tasks WHERE user_id = $1 AND data_reset::date = $2::date AND lower(titulo) = lower($3) LIMIT 1`,
      [userId, String(dataReset).slice(0, 10), titulo]
    ).catch(() => null);
    if (dup) return { tipo, ok: true, titulo, ja: true };
    await run(
      `INSERT INTO tasks (id, titulo, descricao, prioridade, categoria, data_reset, hora, user_id)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`,
      [id, titulo, acao.descricao || '', acao.prioridade || 'media', acao.categoria || 'geral', dataReset, acao.hora || null, userId]
    );
    return { tipo, ok: true, titulo };
  }

  if (tipo === 'marcar_habito') {
    const titulo = String(acao.titulo || 'Academia').trim() || 'Academia';
    const r = await checkinHabito(titulo, userId);
    persistirHistoricoDia(hojeStr(), userId).catch(() => {});
    return {
      tipo,
      ok: true,
      titulo: r.titulo || (r.task && r.task.titulo) || titulo,
      ja: r.ja,
      criada: r.criada
    };
  }

  if (tipo === 'concluir_tarefa') {
    const titulo = String(acao.titulo || acao.nome || '').trim();
    const id = acao.id ? String(acao.id) : null;
    const dataAlvo = (acao.data_reset || acao.data || hojeStr()).slice(0, 10);
    let row = null;
    if (id) row = await get(`SELECT id, titulo, concluida FROM tasks WHERE id = $1 AND user_id = $2`, [id, userId]);
    if (!row && titulo) {
      row = await get(
        `SELECT id, titulo, concluida FROM tasks
         WHERE user_id = $1 AND data_reset::date = $2::date AND concluida = false
           AND (lower(titulo) = lower($3) OR titulo ILIKE $4)
         ORDER BY CASE WHEN lower(titulo) = lower($3) THEN 0 ELSE 1 END
         LIMIT 1`,
        [userId, dataAlvo, titulo, likeContains(titulo)]
      );
    }
    if (!row) {
      return { tipo, ok: false, erro: 'tarefa não encontrada' };
    }
    if (row.concluida) {
      return { tipo, ok: true, titulo: row.titulo, ja: true };
    }
    await run(
      `UPDATE tasks SET concluida = true, concluida_em = COALESCE(concluida_em, CURRENT_TIMESTAMP) WHERE id = $1 AND user_id = $2`,
      [row.id, userId]
    );
    persistirHistoricoDia(hojeStr(), userId).catch(() => {});
    return { tipo, ok: true, titulo: row.titulo, id: row.id };
  }

  if (tipo === 'apagar_tarefa') {
    const dataAlvo = String(acao.data_reset || acao.data || hojeStr()).slice(0, 10);
    if (acao.duplicadas === true || acao.duplicadas === 'true') {
      // Mantém 1 por título (a concluída, se houver); apaga as outras do dia
      const rows = await all(
        `SELECT id, titulo, concluida FROM tasks WHERE user_id = $1 AND data_reset::date = $2::date
         ORDER BY lower(titulo), concluida DESC, id`,
        [userId, dataAlvo]
      );
      const visto = new Set();
      const apagar = [];
      for (const r of rows) {
        const k = String(r.titulo).trim().toLowerCase();
        if (visto.has(k)) apagar.push(r);
        else visto.add(k);
      }
      for (const r of apagar) await run(`DELETE FROM tasks WHERE id = $1 AND user_id = $2`, [r.id, userId]);
      persistirHistoricoDia(hojeStr(), userId).catch(() => {});
      return { tipo, ok: true, duplicadas: true, apagadas: apagar.map((r) => r.titulo) };
    }
    const titulo = String(acao.titulo || acao.nome || '').trim();
    let row = null;
    if (acao.id) row = await get(`SELECT id, titulo FROM tasks WHERE id = $1 AND user_id = $2`, [String(acao.id), userId]);
    if (!row && titulo) {
      row = await get(
        `SELECT id, titulo FROM tasks
         WHERE user_id = $1 AND data_reset::date = $2::date AND (lower(titulo) = lower($3) OR titulo ILIKE $4)
         ORDER BY CASE WHEN lower(titulo) = lower($3) THEN 0 ELSE 1 END, concluida ASC
         LIMIT 1`,
        [userId, dataAlvo, titulo, likeContains(titulo)]
      );
    }
    if (!row) return { tipo, ok: false, erro: 'tarefa não encontrada' };
    await run(`DELETE FROM tasks WHERE id = $1 AND user_id = $2`, [row.id, userId]);
    persistirHistoricoDia(hojeStr(), userId).catch(() => {});
    return { tipo, ok: true, titulo: row.titulo };
  }

  if (tipo === 'criar_evento') {
    const titulo = String(acao.titulo || '').trim();
    const data = String(acao.data || '').slice(0, 10);
    if (!titulo || !/^\d{4}-\d{2}-\d{2}$/.test(data)) {
      return { tipo, ok: false, erro: 'titulo e data (YYYY-MM-DD) obrigatórios' };
    }
    const id = uuid();
    await run(
      `INSERT INTO eventos (id, titulo, descricao, data, hora, tipo, cor, user_id)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`,
      [id, titulo, acao.descricao || '', data, acao.hora || null, acao.tipo_evento || acao.tipo || 'evento', acao.cor || 'blue', userId]
    );
    return { tipo, ok: true, titulo, data, hora: acao.hora || null, id };
  }

  if (tipo === 'criar_alarme') {
    const hora = String(acao.hora || '').trim();
    const mensagem = String(acao.mensagem || acao.titulo || '').trim();
    if (!/^\d{2}:\d{2}$/.test(hora) || !mensagem) {
      return { tipo, ok: false, erro: 'hora (HH:MM) e mensagem obrigatórios' };
    }
    const id = uuid();
    await run(`INSERT INTO alarmes (id, hora, mensagem, user_id) VALUES ($1,$2,$3,$4)`, [id, hora, mensagem, userId]);
    return { tipo, ok: true, hora, mensagem, id };
  }

  return null;
}

module.exports = { TYPES, handle };
