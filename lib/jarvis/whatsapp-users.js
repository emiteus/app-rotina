...
const { get } = requireLib('db');
const { OWNER_LOGIN } = requireLib('plano-owner');
const { normalizeWaId, phoneVariants, phonesAllowed } = requireLib('evolution');
...
function phoneMapStatus() {
  ...
  for (const [phone, login] of map.entries()) {
    ...
    seen.add(login);
    // Nunca expõe o número inteiro, nem pra quem está logado
    entries.push({ login, phoneSample: `…${phone.slice(-4)}` });
  }
  return {
