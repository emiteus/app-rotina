...
function getOsStatus() {
  ...
  return {
    ...
    tools: {
      ...
      critical: tools.filter((t) => t.risk === 'critical').map((t) => t.name),
      byRisk: tools.reduce((acc, t) => {
    ...
    permissionMatrix: {
      documented: true,
      classes: ['AUTO', 'APPROVAL', 'BLOCKED'],
      source: 'TOOL_DEFS + docs/JARVIS_MANUAL.md + npm run check:tools',
      criticalAlwaysHitl: true,
      critical: tools.filter((t) => t.risk === 'critical').map((t) => t.name),
      gap12: 'done'
    },
    ...
    // Fase 2: PCs com o Jarvis Desktop conectados agora
    devices: (() => {
      try {
        return { online: require('./devices/gateway').onlineDeviceIds() };
      } catch (_) {
        return { online: [] };
      }
    })(),
    env: {
      ...
      hitlWhatsapp: !!require('./permissions/engine').hitlAppliesToChannel('whatsapp'),
      proactiveWa: process.env.JARVIS_PROACTIVE_WA !== '0',
      railwayWatch: process.env.JARVIS_RAILWAY_WATCH !== '0',
      opsWatch: process.env.JARVIS_OPS_WATCH !== '0',
      crons: process.env.CRONS_ENABLED !== 'false',
      hitlButtons: process.env.JARVIS_HITL_BUTTONS === '1',
      whisper: process.env.JARVIS_WHISPER !== '0',
      tts: require('./multimodal/tts').ttsReady(),
      ttsMode: require('./multimodal/tts').ttsMode(),
      creative: process.env.JARVIS_CREATIVE !== '0',
      browserOpen: process.env.RESEARCH_FETCH_OPEN === '1'
    }
  };
}
