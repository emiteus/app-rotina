...
/**
 * Fase 5.4: pergunta sobre o que está na tela ("que tá errado aqui?", "lê isso pra mim").
 * No PC vira pc_screen_look sem depender do modelo: com "Ei, Jarvis" na frente ele tratava
 * como cumprimento e pedia o print (24/09/2026). Pedido de print/captura fica com o modelo.
 */
function looksLikeScreenQuestion(mensagem) {
  const soft = stripIngressNoise(mensagem)
    .normalize('NFD')
    .replace(/\p{M}/gu, '')
    .toLowerCase()
    ...
    .replace(/\s+/g, ' ')
    .trim();
  if (/\b(print|printa|captura|screenshot)\b/.test(soft)) return false;
  return [
    /\b(o )?que (e que )?(ta|esta|tem|ha) (de )?errad[oa]\b/,
    /\b(que|qual) (e )?(o |esse |essa |este )?(erro|problema|aviso|bug)( e)? (esse|essa|aqui|ai|na tela)\b/,
    /\b(que|qual) (erro|problema|aviso|bug) (e esse|e essa|e aquele|ta dando|esta dando)\b/,
    /\b(le|leia|ler) (isso|isto|aqui|esse|essa|o que ta|o que esta|pra mim|para mim)\b/,
    /\bo que (e|eh|significa|quer dizer) (isso|isto|esse|essa)( aqui| ai)?\b/,
    /\bo que (tem|ta|esta|aparece|voce ve|vc ve) na (minha )?tela\b/,
    /\b(olha|olhe|ve|veja|le|leia|analisa|analise|confere|confira) (a |minha |a minha )?tela\b/,
    /\bna minha tela\b/,
    /\b(olha|olhe|ve|veja|confere|confira) (isso|isto|aqui|esse|essa)\b/,
    /\bda (uma )?olhada (nisso|aqui|nesse|nessa|na tela|na minha tela)\b/,
    /\b(explica|explique|traduz|traduza|resume|resuma) (isso|isto|aqui|esse|essa)\b/,
    /\bme ajuda (com isso|com esse|com essa|nisso|aqui)\b/,
    /\b(ta|esta) vendo (isso|aqui|esse|essa)\b/,
    /\bo que (voce|vc) (ta|esta) vendo\b/
  ].some((re) => re.test(soft));
}

module.exports = { detectIntent, looksLikeGreeting, looksLikeCommand, looksLikeScreenQuestion, stripIngressNoise };
