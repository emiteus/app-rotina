...
function slimProjetos(projetos, hints) {
  ...
  const keys = hints && hints.length
    ? hints
    : ['cinerush', 'cinerush_editor', 'attracione', 'socialhub', 'clipper', 'projeto_milhao'];
  const out = {};
  for (const k of keys) {
    const p = projetos[k];
    if (!p) continue;
    ...
    if (copy.hoje?.total != null && copy.hoje.por_pessoa) {
      // keep as-is (CineRush Editor stats)
    }
  ...
  for (const k of ['cinerush', 'cinerush_editor', 'attracione', 'socialhub', 'clipper', 'projeto_milhao', 'cutflix']) {
    if (out[k]) continue;
    const p = projetos[k];
