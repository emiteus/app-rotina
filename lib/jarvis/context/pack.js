/**
 * Snapshot → prompt pack (Phase 4).
 * Full snap still used for local action inference; LLM gets a trimmed pack.
 */
...
function slimProjetos(projetos, hints) {
  ...
  for (const k of keys) {
    ...
    // Keep summary-ish fields; drop huge nested dumps
    const copy = { ...p };
  ...
  // Always show connectivity map for non-hinted modules
