...
function resolveTargetProject(mensagem) {
  ...
  const p =
    ...
    (/\b(socialhub|social\s*hub|teuhub|teu\s*hub|teushub|teus\s*hub)\b/i.test(mensagem)
      ? resolveProject('socialhub')
      : null);
  if (p && PROJECT_FLAGS[p.id]) return p.id;
  if (/\b(cine\s*rush|cinerush|cinehub|havok|chatwoot)\b/i.test(mensagem)) {
    return 'cinerush';
  }
  if (/\b(socialhub|social\s*hub|teuhub|teu\s*hub|teushub|teus\s*hub)\b/i.test(mensagem)) {
    return 'socialhub';
  }
  return null;
}
...
function pickKeys(mensagem, projectId) {
  ...
  if (
    (/\b(cron|publish|publica|agendad|token|post)\b/i.test(t) ||
      /\b(socialhub|teuhub|teu\s*hub)\b/i.test(t)) &&
    known.includes('crons')
  ) {
    picked.add('crons');
  }
  ...
  const allHint =
    ...
    /\btudo\b/i.test(t) ||
    /\btodas?\b/i.test(t) ||
    /\bpaus[ae].{0,40}(cine|havok|suporte|acesso|social|teuhub|cron)\b/i.test(t);
...
function inferOpsFlagActionsFromMessage(mensagem) {
  ...
  const opsContext =
    /\b(automa|flag|suporte|acesso|provision|havok|chatwoot|cinehub|bot|e-?mail|cron|publish|socialhub|teuhub)\b/i.test(
      t
    );
