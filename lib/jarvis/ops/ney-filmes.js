...
function nomeDaObra(arquivo) {
  return String(arquivo || '')
    ...
    .replace(/[._]+/g, ' ')
    .replace(/\((\d{1,2})\)|\[[^\]]*\]/g, ' ')
    .replace(/\b(1080p|720p|480p|2160p|4k|hd|fhd|x264|x265|hevc|web-?dl|bluray|dublado|legendado|corte|cortes|partes?[\s-]*\d+|part[\s-]*\d+|pt[\s.-]*\d+|final|v\d)\b/gi, ' ')
    .replace(/\s*-\s*\d+\s*$/, '')
    .replace(/\s{2,}/g, ' ')
    .replace(/[\s\-–—]+$/, '')
    .trim();
...
/** "Tropa de Elite parte 2.mp4" / "pt2" / "part-3" → 2 / 2 / 3; sem parte → null */
function parteDe(arquivo) {
  const m = String(arquivo || '').match(/\b(?:partes?|part|pt)[\s.\-_]*(\d{1,3})\b/i);
  return m ? Number(m[1]) : null;
}

/**
 * Ordem de postagem: as partes da mesma obra ficam juntas e em ordem (parte 1 antes da 2),
 * na posição da primeira vez que a obra aparece na pasta.
 */
function ordenarPartes(videos) {
  const chave = (v) => nomeDaObra(v.nome).toLowerCase();
  const primeira = new Map();
  videos.forEach((v, i) => { if (!primeira.has(chave(v))) primeira.set(chave(v), i); });
  return videos
    .map((v, i) => ({ v, i }))
    .sort((a, b) =>
      primeira.get(chave(a.v)) - primeira.get(chave(b.v)) ||
      (parteDe(a.v.nome) ?? 0) - (parteDe(b.v.nome) ?? 0) ||
      a.i - b.i)
    .map((x) => x.v);
}

function legenda({ sinopse, tipo, titulo, parte }) {
  return `${String(sinopse).trim()}\n\n🎬 ${tipo}: ${String(titulo).trim()}${parte ? ` (Parte ${parte})` : ''}`;
}
...
async function postarVideos(userId, videos, deps = {}) {
  ...
  const agendados = [];
  const falhas = [];
  videos = ordenarPartes(videos);
  // Partes da mesma obra: busca a sinopse uma vez só (mesma sinopse nas partes e menos espera)
  const sinopses = new Map();
  for (const [i, v] of videos.entries()) {
    try {
      const chave = nomeDaObra(v.nome).toLowerCase();
      if (!sinopses.has(chave)) sinopses.set(chave, await sinopseDe(v.nome, deps));
      const obra = { ...sinopses.get(chave), parte: parteDe(v.nome) };
      const up = await sh.assinarUpload();
      const r = await requestTool(userId, 'pc_media_upload', { arquivo: v.arquivo, upload: { url: up.url, fields: up.fields } }, { timeoutMs: 30 * 60 * 1000 }); // compressão lenta (qualidade) + upload
      ...
      agendados.push({ arquivo: v.nome, tipo: obra.tipo, titulo: obra.parte ? `${obra.titulo} (Parte ${obra.parte})` : obra.titulo, achou: obra.achou, quando: quandoTxt(quandos[i]) });
    } catch (e) {
      if (e && e.semCredito) {
