...
function targets() {
  ...
  // TeusHub: agora é o Jarvis que publica os agendados dele — se cair, os posts param
  const th = String(process.env.JARVIS_UPTIME_TEUSHUB_URL || 'https://teushub.online/login').trim();
  if (th && process.env.JARVIS_UPTIME_TEUSHUB !== '0') list.push({ key: 'teushub', name: 'o TeusHub', url: th, host: 'railway' });
  return list;
}
