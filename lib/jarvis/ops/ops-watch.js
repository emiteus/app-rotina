...
async function checkOpsHealth(userId = null) {
    ...
    if (ed.conectado === false && process.env.CINERUSH_EDITOR_URL) {
      ...
      if (remember(key)) {
        notes.push({
          ...
          speech: 'O Editor do CineRush está fora do ar.'
        });
      }
      ...
      try {
        const { recordSample } = require('../analytics/series');
        if (q) {
          await recordSample('cinerush_editor', 'queue_waiting', q.waiting || 0, q);
          await recordSample('cinerush_editor', 'queue_failed', q.failed || 0, q);
          await recordSample('cinerush_editor', 'queue_active', q.active || 0, q);
        }
      } catch {
        /* analytics opcional */
      }
      if (q && q.failed > 0) {
        ...
        if (remember(key)) {
          notes.push({
            ...
            speech: `O Editor do CineRush tem ${q.failed} ${q.failed === 1 ? 'corte que falhou' : 'cortes que falharam'} na fila.`
          });
        }
        ...
        if (remember(key)) {
          notes.push({
            ...
            text:
              ...
              ` (limite ${thr}).`,
            speech: `A fila do Editor do CineRush está com ${q.waiting} cortes esperando.`
          });
        }
      }
    }
  ...
  if (cr && cr.conectado) {
    ...
    try {
      ...
      if (typeof credits === 'number' && credits >= 0) {
        await recordSample('cinerush', 'havok_credits', credits, {
          cached: !!(cr.havok && cr.havok.cached)
        });
      }
      const pend = cr.assinantes?.pendentes;
      if (typeof pend === 'number') {
        await recordSample('cinerush', 'assinantes_pendentes', pend);
      }
    } catch {
      /* analytics opcional */
    }
    if (typeof credits === 'number' && credits >= 0 && credits < thr) {
      ...
      if (remember(key)) {
        notes.push({
          ...
          speech: `O Havok do CineRush está com só ${credits} ${credits === 1 ? 'crédito' : 'créditos'}.`
        });
      }
    }
    const pend = cr.assinantes?.pendentes;
    ...
    if (typeof pend === 'number' && pend >= pendThr) {
      ...
      if (remember(key)) {
        notes.push({
          ...
          speech: `O CineRush tem ${pend} ${pend === 1 ? 'assinante esperando' : 'assinantes esperando'} liberação.`
        });
      }
    }
  }
    ...
    if (at.operacao?.backup_atrasado) {
      if (remember('attracione_backup')) {
        notes.push({
          ...
          speech: 'O backup da Attracione está atrasado.'
        });
      }
    }
    ...
    if (contas.length) {
      ...
      if (remember(`attracione_contas:${contas.length}:${nomes}`)) {
        notes.push({
          ...
          speech: `${contas.length === 1 ? 'Uma conta da Attracione está' : `${contas.length} contas da Attracione estão`} com problema: ${nomes}.`
        });
      }
    }
    ...
    if (quebradas.length) {
      ...
      if (remember(`attracione_plat:${list}`)) {
        notes.push({
          ...
          speech: `A coleta da Attracione quebrou em: ${list}.`
        });
      }
    }
  }
  ...
  // Séries históricas → alertas genéricos (dedupe via remember)
  try {
    const { runGenericMetricAlerts } = require('../analytics/series');
    const metricNotes = await runGenericMetricAlerts();
    for (const n of metricNotes || []) {
      const key = `metric:${n.project_id}:${n.metric}:${n.value}`;
      if (remember(key)) notes.push(n);
    }
  } catch {
    /* analytics opcional */
  }
