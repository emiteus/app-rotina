...
const ensureMissionsTable = once(async () => {
  await run(`
...
async function getActiveMission(userId) {
  await ensureMissionsTable();
  // Inclui `failed` p/ status/retry acharem a missão que acabou de falhar
  // (senão cai numa planned antiga e "retry passo" erra o alvo).
  const row = await get(
    `SELECT * FROM jarvis_missions
     WHERE user_id = $1 AND status IN ('draft','planned','running','waiting_approval','paused','failed')
...
/**
 * CAS: planned/paused → running. Só um avanço por vez.
 * @returns {object|null} missão claimed ou null se conflito
 */
async function claimMissionRun(id, userId) {
  await ensureMissionsTable();
  const row = await get(
    `UPDATE jarvis_missions
     SET status = 'running', atualizado_em = CURRENT_TIMESTAMP
     WHERE id = $1 AND user_id = $2 AND status IN ('planned', 'paused')
     RETURNING *`,
    [id, userId]
  );
  if (!row) return null;
...
async function updateMission(id, userId, patch) {
  ...
  const saved = await getMission(id, userId);
  // Fase 1: transição pra "done" = caminho provado → receita (todas as rotas de conclusão passam aqui)
  if (saved && saved.status === 'done' && cur.status !== 'done') {
    require('../learning/recipes')
      .saveRecipeFromMission(userId, saved)
      .catch((e) => console.error('[jarvis.recipe] save:', e.message));
  }
  return saved;
}
