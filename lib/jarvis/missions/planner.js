 ...
 * Retorna texto extra se avançou; null se não havia missão esperando.
 */
async function resumeMissionAfterApproval(userId, approvalAcoes = []) {
  ...
  return {
    text:
      `Missão **${mission.id}** passo ${idx + 1} confirmado.` +
      (more
        ? `\nManda **próximo passo** ou **executa missão** pra seguir.\n\n${formatMission(m2)}`
        : `\nMissão concluída.\n\n${formatMission(m2)}`),
    mission: m2,
    canAdvance: more
  };
