...
function planFromGoal(goal) {
  ...
  if (
    ...
    else if (/cutflix/i.test(g)) project = 'cutflix';
    else if (/jarvis/i.test(g)) project = 'jarvis';
    const topic = g
    ...
    steps.push({
      ...
      args: {
        tipo: 'research_web_search',
        query: `lançar produto ${topic}`.slice(0, 160),
        limit: 5
      }
    });
    steps.push({
...
function parseMissionCommand(mensagem) {
  ...
  if (
    /^(pr[oó]ximo\s+passo|pr[oó]ximo|continua(\s+miss[aã]o)?|seguir(\s+miss[aã]o)?|roda\s+miss[aã]o|avança|avancar|bora|mete\s+marcha(\s+(na\s+)?miss[aã]o)?)\s*[!.]*$/i.test(
      t
    )
  ) {
    return { cmd: 'advance' };
  ...
  if (
    /^(executa\s+miss[aã]o|roda\s+tudo|completa\s+miss[aã]o|run\s+mission|executar\s+miss[aã]o|mete\s+marcha\s+(n[oa]\s+)?(tudo|batch))(?:\s+([a-f0-9]{6,12}))?\s*[!.]*$/i.test(
      t
    )
  ) {
    const id = t.match(/([a-f0-9]{6,12})\s*$/i);
