...
function planFromGoal(goal) {
  ...
  if (
    ...
    const topic = g
      ...
      .trim()
      .slice(0, 120) || project;
    // Query amarrada no brief — evita hits genéricos (ex.: Cutflix haircut YouTube)
    let researchQuery = `lançar produto ${topic}`.slice(0, 160);
    try {
      ...
      if (b) {
        ...
        const gist = String(b.brief || '')
          ...
          .trim()
          .slice(0, host ? 90 : 110);
        researchQuery = [b.name || project, host, gist, 'SaaS video AI']
          .filter(Boolean)
          .join(' ')
          .replace(/\s+/g, ' ')
          .trim()
