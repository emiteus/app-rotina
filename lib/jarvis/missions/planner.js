...
function planFromGoal(goal) {
  ...
  // Gap #9 — "prepara landing" → research → copy real → (visual) → browser → checklist
  if (!steps.length && looksLikeLandingPrep(g)) {
    ...
    try {
      const { getBrief } = require('../projects/briefs');
      const { cleanLandingTopic } = require('../multimodal/landing-copy');
      topic = cleanLandingTopic(topic, project, getBrief(project) || { name: project }) || project;
    } catch (_) {
      topic = topic || project;
    }
    ...
    steps.push({
      description: `Research refs/concorrentes: ${(topic && topic !== project ? topic : project).slice(0, 50)}`,
      tipo: 'research_web_search',
    ...
    steps.push({
      ...
      args: {
        tipo: 'creative_landing_copy',
        project,
        topic: topic && topic !== project ? topic : undefined
      }
    });
...
async function failuresHintForGoal(userId, goal, { tools = [] } = {}) {
  const lines = [];
  // Lições (Fase 1): falhas repetidas por tool/projeto + o que resolveu da última vez
  try {
    const { resolveProjectsFromMessage } = require('../projects/registry');
    const { relevantLessons, formatLessonLine } = require('../learning/lessons');
    const lessons = await relevantLessons(userId, {
      tools: tools.filter(Boolean),
      projectIds: resolveProjectsFromMessage(goal) || [],
      limit: 5
    });
    for (const l of lessons) lines.push(formatLessonLine(l));
  } catch (_) {
    /* sem lições → segue só com ultima_falha */
  }
  try {
    const { loadMemoriesForMessage } = require('../memory/projects');
    const mems = await loadMemoriesForMessage(userId, goal, { limit: 6 });
    // slimMemory já traz string; aceita {t,text} por segurança
    const fails = (mems || [])
      .map((m) => m.ultima_falha && (m.ultima_falha.text || m.ultima_falha))
      .filter((f) => typeof f === 'string' && f.trim())
      .slice(0, 3);
    for (const f of fails) {
      if (!lines.some((l) => l.includes(f.slice(0, 40)))) lines.push(`- ${f}`);
    }
  } catch (_) {
    /* ignore */
  }
 ...
 */
async function startMission(userId, goal, { ignoreRecipes = false } = {}) {
  // Fase 1: caminho que já deu certo pra um objetivo parecido (mesmos projetos)
  if (!ignoreRecipes) {
    try {
      const recipe = await require('../learning/recipes').findRecipeForGoal(userId, goal);
      if (recipe && recipe.steps.length) {
        const m = await createMission(userId, goal, normalizeSteps(recipe.steps));
        return m ? { ...m, recipe: { id: recipe.id, name: recipe.name, uses: recipe.uses } } : m;
      }
    } catch (e) {
      console.log(JSON.stringify({ tag: 'jarvis.recipe', event: 'lookup_fail', erro: e.message }));
    }
  }
...
function parseMissionCommand(mensagem) {
  ...
  if (/^(refaz(er)?\s+(o\s+)?plano|plano\s+novo|ignora\s+(a\s+)?receita)\s*[!.]*$/i.test(t)) {
    return { cmd: 'replan' };
  }
  ...
  // Palavra solta ("bora", "continua") é conversa comum — só vale com missão recente
  if (/^(pr[oó]ximo|continua|seguir|avança|avancar|bora)\s*[!.]*$/i.test(t)) {
    return { cmd: 'advance', loose: true };
  }
  if (
...
function formatMission(mission) {
  ...
  const lines = steps
    ...
    .join('\n');
  const viaReceita = mission.recipe
    ? `Montei pela receita **${mission.recipe.name}** (deu certo ${mission.recipe.uses}x). ` +
      `Prefere plano do zero? Manda **refaz plano**.\n`
    : '';
  return (
...
const LOOSE_COMMAND_MAX_AGE_MS = 6 * 60 * 60 * 1000;

function missionAgeMs(mission) {
  const t = mission && mission.atualizado_em ? new Date(mission.atualizado_em).getTime() : NaN;
  return Number.isFinite(t) ? Date.now() - t : Infinity;
}

/**
 * "running" há mais que o maior timeout de tool = processo caiu no meio (deploy/restart).
 * Vira failed com aviso honesto — senão a missão fica travada pra sempre.
 */
async function failIfStaleRunning(mission, userId) {
  if (!mission || mission.status !== 'running' || missionAgeMs(mission) < STALE_RUNNING_MS) {
    return null;
  }
  const steps = [...(mission.steps || [])];
  const idx = Math.min(Math.max(mission.currentStep || 0, 0), Math.max(steps.length - 1, 0));
  const erro = 'interrompido (reinício/deploy no meio do passo) — pode ter rodado; confere antes de repetir';
  if (steps[idx] && steps[idx].status === 'pending') {
    steps[idx] = { ...steps[idx], status: 'failed', result: { ok: false, uncertain: true, erro } };
  }
  await updateMission(mission.id, userId, {
    steps,
    status: 'failed',
    currentStep: idx,
    result: { ok: false, uncertain: true, erro }
  });
  console.log(JSON.stringify({ tag: 'jarvis.mission', event: 'stale_running', id: mission.id, step: idx + 1 }));
  const m = await getMission(mission.id, userId);
  return {
    ok: false,
    outcome: 'fail',
    stepIndex: idx,
    resposta:
      `Missão **${mission.id}** tinha parado no passo ${idx + 1} (${erro}).\n` +
      `Manda **retry passo** quando conferir.`,
    acoes: [],
    mission: m
  };
}
...
async function advanceMission(userId, executeAcoes, missionId = null) {
  ...
  if (mission.status === 'running') {
    const stale = await failIfStaleRunning(mission, userId);
    if (stale) return stale;
    return {
      ok: false,
      resposta: `Missão **${mission.id}** já está em execução — espera o passo atual.`
    };
  }
  if (mission.status === 'waiting_approval') {
    return {
      ok: false,
      resposta:
        `Missão **${mission.id}** espera **SIM** pra continuar.\n` + formatMission(mission)
    };
  }

  // CAS planned/paused → running (duas mensagens simultâneas não avançam juntas)
  const claimed = await claimMissionRun(mission.id, userId);
  if (!claimed) {
    const again = await getMission(mission.id, userId);
    if (again && again.status === 'running') {
      return {
        ok: false,
        resposta: `Missão **${mission.id}** já está em execução (outra mensagem).`
      };
    }
    return {
      ok: false,
      resposta: `Não consegui travar a missão **${mission.id}** — tenta de novo.`
    };
  }
  mission = claimed;
  ...
  // currentStep alinhado ao claim (já running)
  await updateMission(mission.id, userId, { status: 'running', currentStep: idx });
  mission = (await getMission(mission.id, userId)) || mission;
...
async function tryHandleMissionCommand(userId, mensagem, executeAcoes, onProgress = null) {
  ...
  if (parsed.cmd === 'replan') {
    const active = await getActiveMission(userId);
    if (!active || active.status !== 'planned' || (active.steps || []).some((s) => s.status !== 'pending')) {
      return {
        handled: true,
        resposta: 'Só dá pra refazer o plano de uma missão que ainda não começou.',
        acoes: [],
        provider: 'mission'
      };
    }
    await cancelMission(active.id, userId);
    const m = await startMission(userId, active.goal, { ignoreRecipes: true });
    return {
      handled: true,
      resposta: `Plano refeito do zero (a missão ${active.id} foi cancelada).\n\n${formatMission(m)}`,
      acoes: [],
      provider: 'mission'
    };
  }
  ...
  if (parsed.cmd === 'advance') {
    ...
    // "bora"/"continua" soltos: só com missão viva e recente; senão é conversa normal
    if (
      parsed.loose &&
      (!active ||
        !['planned', 'paused', 'waiting_approval', 'running'].includes(active.status) ||
        missionAgeMs(active) > LOOSE_COMMAND_MAX_AGE_MS)
    ) {
      return null;
    }
...
/**
 * O SIM aprovado é o do passo? Compara o approval_id guardado no passo;
 * missões antigas (sem id no result) caem no match por tipo da tool.
 */
function approvalMatchesStep(step, approvalId, approvalAcoes) {
  const stepApproval = step && step.result && step.result.approval_id;
  if (stepApproval) return !!approvalId && stepApproval === approvalId;
  return (approvalAcoes || []).some((a) => a && a.tipo === step.tipo);
}

/**
 * Após SIM no HITL: marca o passo waiting_approval como done e libera a missão.
 * Só se o SIM for do próprio passo — SIM de outro pedido não conclui passo que não rodou.
 * Retorna canAdvance=true se ainda há passos — o host deve chamar runMissionBatch.
 */
async function resumeMissionAfterApproval(userId, approvalAcoes = [], { approvalId = null } = {}) {
  ...
  if (!approvalMatchesStep(steps[idx], approvalId, approvalAcoes)) {
    console.log(
      JSON.stringify({
        tag: 'jarvis.mission',
        event: 'resume_skip_other_approval',
        id: mission.id,
        step: idx + 1,
        approvalId
      })
  ...
  const stepTipo = steps[idx].tipo;
  const sameTipo = (a) => a && (!stepTipo || a.tipo === stepTipo);
  const doneAcao = (approvalAcoes || []).find((a) => sameTipo(a) && a.ok && !a.pending_approval);
  const failedAcao = (approvalAcoes || []).find(
    (a) => sameTipo(a) && a.ok === false && !a.pending_approval
  );
