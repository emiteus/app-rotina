...
- Página/URL (“abre/olha esse link”) → browser_open / browser_links
- Imagem/banner/arte → creative_generate_image (prompt claro)
- Ajuste de layout existente (“afasta”, “aproxima”, “maior”) → NÃO inventar arte nova; precisa da imagem de referência (usuário responde à imagem ou reenvia o print)
...
- Pós-PR: dev_deploy_checklist (orientação; não redeploy sozinho).
...
- NÃO emita tools de write financeiro/deploy.`
  ...
  browser: {
    id: 'browser',
    name: 'Browser',
    aliases: ['browser', 'navegador', 'web'],
    toolPrefixes: ['browser_'],
    // Sessão headless (Map em memória) precisa sobreviver entre turnos — worker morre no fim
    inProcess: true,
    addendum: `Agente Browser ativo:
- Use browser_open pra snapshot (HTTP). SPA/JS: browser_open com headless:true (JARVIS_BROWSER_HEADLESS=1).
- Depois: browser_click / browser_type / browser_snapshot na mesma sessão.
- Use browser_links se pedirem só a lista de links.
- Allowlist SSRF (RESEARCH_FETCH_ALLOWLIST / OPEN=1) — se host bloqueado, diga.
- NÃO invente conteúdo da página — só o que a tool devolver.`
  },
  creative: {
    id: 'creative',
    name: 'Creative',
    aliases: ['creative', 'criativo', 'imagem', 'arte'],
    toolPrefixes: ['creative_'],
    addendum: `Agente Creative ativo:
- Imagem/banner/arte → creative_generate_image (prompt claro; aspect 16:9 banner, 9:16 story).
- Outline de landing (hero/CTA/seções) → creative_landing_copy com project=id do brief.
- NÃO invente que gerou se a tool falhou (quota Gemini).`
  },
  rotina: {
    id: 'rotina',
    name: 'Rotina',
...
function parseAgentCommand(mensagem) {
  ...
  m = t.match(
    /^\/(finance|ops|dev|debug|research|browser|rotina|financeiro|pesquisa|diagnostico|navegador)\s*(.*)$/i
  );
  if (m) {
    const map = {
      finance: 'finance',
...
function looksLikeCreativeRequest(mensagem) {
  const t = String(mensagem || '');
  return /\b(gera|gerar|cria|criar|desenha|faz)\b.{0,40}\b(imagem|foto|banner|arte|illustration|ilustra)/i.test(
    t
  );
}

function looksLikeBrowserRequest(mensagem) {
  const t = String(mensagem || '');
  if (looksLikeDevRequest(t) || looksLikeCreativeRequest(t)) return false;
  // "abre X no meu PC" = abrir no navegador DO PC (pc_open_url), não ler a página no servidor
  if (/\b(no|na|pelo|pela)\s+(meu\s+|minha\s+)?(pc|computador|notebook|m[aá]quina|desktop|navegador\s+do\s+pc)\b/i.test(t)) return false;
  return (
    /\b(abre|abrir|olha|olhe|visita|visite|navega|navegue|snapshot)\b.{0,40}\b(https?:\/\/\S+|www\.\S+)/i.test(
      t
    ) ||
    /\b(o\s+que\s+tem|o\s+que\s+aparece|resumo\s+d[aeo]|lista\s+os\s+links)\b.{0,60}\b(https?:\/\/\S+|www\.\S+|nessa\s+p[aá]gina|nessa\s+url|desse\s+site)\b/i.test(
      t
    ) ||
    /\bbrowser_(open|links)\b/i.test(t) ||
    (/^https?:\/\/\S+$/i.test(t.trim()) && t.trim().length < 300)
  );
}
