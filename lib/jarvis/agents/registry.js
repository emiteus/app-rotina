...
- Código: dev_read_file → SEMPRE o conteúdo vem no texto da tool (cole no WA). Depois propose_patch → apply/PR (HITL).
...
function looksLikeDevRequest(mensagem) {
  ...
  return (
    ...
    /\b(patch|aplica\s+patch|abre\s+(um\s+)?pr|pull\s*request|implementa|corrige\s+o\s+c[oó]digo|refatora)\b/i.test(
      t
    )
  );
}
