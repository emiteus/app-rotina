...
- Use research_web_search com a query do usuário.
- Se precisar de detalhe, research_fetch_url nas melhores URLs (allowlist).
- Feche com research_write_report (title + body/findings + sources) pra devolver o relatório no WA.
- NÃO invente URLs, quotes ou números — só o que as tools devolverem.
...
function pickAgentForMessage(mensagem) {
  ...
  if (looksLikeResearchRequest(mensagem)) {
    return { agent: getAgent('research'), message: mensagem, explicit: false };
  }
