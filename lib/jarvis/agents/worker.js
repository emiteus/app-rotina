...
function forkJob({ userId, agentId, acoes, timeoutMs }) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`agent worker timeout após ${ms}ms (${agentId || 'default'})`));
    }, ms);
    ...
    child.on('message', (msg) => {
      ...
      if (msg.type === 'done') finish(null, msg.results || []);
      else if (msg.type === 'error') finish(new Error(msg.erro || 'worker error'));
    });
    ...
    child.on('exit', (code, signal) => {
      ...
      finish(
        new Error(
          `agent worker exit code=${code} signal=${signal || ''} (${agentId || 'default'})`
        )
      );
    });
...
async function runScopedBatches({
  ...
  if (workersEnabled() && agentId) {
    try {
      ...
      const prefixes = getAgent(agentId).toolPrefixes || [];
      useWorker = prefixes.length > 0;
    } catch {
      useWorker = false;
    }
  }
