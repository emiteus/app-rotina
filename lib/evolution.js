...
function textoParaWhatsApp(md) {
  return String(md || '')
    // Etiquetas de CONTEÚDO EXTERNO ficam no histórico (proteção do modelo), não na tela dele (24/09/2026)
...
/**
 * Envia imagem (base64 ou URL). Evolution sendMedia.
 */
async function sendWhatsAppImage(to, imageBase64, opts = {}) {
  if (!evolutionReady()) {
    throw new Error('Evolution não configurada (EVOLUTION_URL/API_KEY/INSTANCE)');
  }
  const number = normalizeWaId(to);
  if (!number) throw new Error('número inválido');
  let media = String(imageBase64 || '').replace(/^data:[^;]+;base64,/, '').replace(/\s+/g, '');
  if (!media) throw new Error('imagem vazia');

  const mime = opts.mime || 'image/png';
  const caption = textoParaWhatsApp(opts.caption || '').slice(0, 900);
  const fileName = opts.fileName || `jarvis-${Date.now()}.png`;
  const instance = process.env.EVOLUTION_INSTANCE;
  const url = `${evolutionBase()}/message/sendMedia/${instance}`;
  const headers = {
    apikey: process.env.EVOLUTION_API_KEY,
    'Content-Type': 'application/json'
  };
      ...
      if (res.ok) {
        console.log(
          JSON.stringify({
            ...
            event: 'image_sent',
            number,
            bytesApprox: Math.floor((media.length * 3) / 4)
          })
        );
        return data;
      }
      lastErr = new Error(data?.message || data?.error || `Evolution media HTTP ${res.status}`);
    } catch (e) {
      lastErr = e;
    }
  }
  throw lastErr || new Error('Evolution image falhou');
}
