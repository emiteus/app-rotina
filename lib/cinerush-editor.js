...
async function getCinerushEditorSnapshot() {
  ...
  try {
    ...
    const hoje = stats?.hoje
      ? {
          data: stats.hoje.data || null,
          total: stats.hoje.total ?? 0,
      ...
      : {
          ...
          total: null,
          por_pessoa: [],
          motivo: stats?.erro || 'stats sem bloco hoje — confira /api/ops/stats'
        };
    return {
      conectado: !!health.ok,
      ...
      hoje,
      nota:
        'Posts IG do dia → hoje.por_pessoa. CineRush Editor ≠ CineRush TV (IPTV) ≠ Attracione (comp).',
...
/** Trabalhos recentes (memória do servidor, ~1 h). status: error | running | queued | done | all */
async function listJobs({ status = '', user = '', limit = 20 } = {}) {
  const q = new URLSearchParams({ limit: String(limit) });
  if (status) q.set('status', status);
  if (user) q.set('user', user);
  return editorFetch(`/api/ops/jobs?${q}`, { timeoutMs: 15000 });
}

/** Posts no banco. status: scheduled | failed | posted */
async function listScheduled({ status = 'scheduled', user = '', limit = 30 } = {}) {
  const q = new URLSearchParams({ status, limit: String(limit) });
  if (user) q.set('user', user);
  return editorFetch(`/api/ops/scheduled?${q}`, { timeoutMs: 20000 });
}
