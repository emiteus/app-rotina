...
async function initDB() {
  try {
    ...
    `);
    // Push Notifications — fica FORA do cache 24h pra garantir criação ao subir feature
    await pool.query(`
    ...
    `);
    // Despesas do mês — fora do cache 24h (feature nova)
    await pool.query(`
    ...
    `);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_despesas_mes_ym ON despesas_mes (ym)`).catch(() => {});
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_despesas_mes_status ON despesas_mes (ym, status)`).catch(() => {});
    // Receitas do mês — cadastro manual (não mistura com entradas do banco)
    await pool.query(`
      CREATE TABLE IF NOT EXISTS receitas_mes (
        id TEXT PRIMARY KEY,
        ym TEXT NOT NULL,
        titulo TEXT NOT NULL,
    ...
    `);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_receitas_mes_ym ON receitas_mes (ym)`).catch(() => {});
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_receitas_mes_status ON receitas_mes (ym, status)`).catch(() => {});
    // Sprint 6: coluna pra guardar quando o Pluggy vai auto-sync o item (null = manual/Meu Pluggy)
    await pool.query(`ALTER TABLE openfinance_items ADD COLUMN IF NOT EXISTS next_auto_sync TIMESTAMP`).catch(() => {});
    // Horário real de conclusão — fora do cache 24h
    await pool.query(`ALTER TABLE tasks ADD COLUMN IF NOT EXISTS concluida_em TIMESTAMP`).catch(() => {});
    // Categorias novas — fora do cache 24h
    await pool.query(`INSERT INTO categorias (chave, label) VALUES ('projetos','Projetos') ON CONFLICT (chave) DO NOTHING`).catch(() => {});
    await pool.query(`INSERT INTO categorias (chave, label) VALUES ('faturas','Faturas') ON CONFLICT (chave) DO NOTHING`).catch(() => {});
    await pool.query(`INSERT INTO categorias (chave, label) VALUES ('iof','IOF') ON CONFLICT (chave) DO NOTHING`).catch(() => {});
    // Histórico do Assistente — fora do cache 24h
    await pool.query(`
      CREATE TABLE IF NOT EXISTS assist_conversas (
        id TEXT PRIMARY KEY,
        titulo TEXT,
        criado_em TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        atualizado_em TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    ...
    `);
    await pool.query(`
      CREATE TABLE IF NOT EXISTS assist_mensagens (
        id TEXT PRIMARY KEY,
        conversa_id TEXT NOT NULL REFERENCES assist_conversas(id) ON DELETE CASCADE,
        role TEXT NOT NULL,
        content TEXT NOT NULL,
        criado_em TIMESTAMP DEFAULT CURRENT_TIMESTAMP
      )
    `);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_assist_msgs_conversa ON assist_mensagens (conversa_id, criado_em)`).catch(() => {});
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_assist_conversas_atualizado ON assist_conversas (atualizado_em DESC)`).catch(() => {});

    // Multi-usuário — sempre roda (fora do cache 24h)
    try {
      const { migrarMultiUsuario } = require('./migrate-multiuser');
      await migrarMultiUsuario();
    } catch (e) {
      console.error('[multiuser] erro na migração:', e.message);
    }
    ...
    await pool.query(`ALTER TABLE financeiro ADD COLUMN IF NOT EXISTS pago_terceiro BOOLEAN DEFAULT false`).catch(() => {});
    await pool.query(`ALTER TABLE financeiro ADD COLUMN IF NOT EXISTS terceiro_nome TEXT`).catch(() => {});
    await pool.query(`ALTER TABLE financeiro ADD COLUMN IF NOT EXISTS terceiro_notas TEXT`).catch(() => {});
    ...
    const seed = [
      ...
      ['outros','Outros'], ['projetos','Projetos'], ['faturas','Faturas'], ['iof','IOF']
    ];
    ...
    await pool.query(`ALTER TABLE openfinance_items ADD COLUMN IF NOT EXISTS ultima_refresh_pedido TIMESTAMP`).catch(() => {});
    await pool.query(`ALTER TABLE openfinance_items ADD COLUMN IF NOT EXISTS ultimo_status TEXT`).catch(() => {});
