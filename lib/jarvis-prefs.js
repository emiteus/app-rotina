...
async function saveJarvisPrefs(userId, patch) {
  ...
  // Notas: merge por texto (patchPrefs às vezes manda lista só com a nota nova)
  if (Array.isArray(patch.extras?.notas)) {
    const incoming = patch.extras.notas;
    const existing = Array.isArray(cur.extras?.notas) ? cur.extras.notas : [];
    const byKey = new Map();
    for (const n of [...incoming, ...existing]) {
      ...
      byKey.set(key, typeof n === 'string' ? { t: new Date().toISOString(), text: n } : n);
    }
    next.extras.notas = [...byKey.values()].slice(0, 20);
  }
  await run(
